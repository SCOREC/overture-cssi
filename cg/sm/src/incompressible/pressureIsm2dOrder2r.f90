! This file automatically generated from pressureIsmOpt.bf90 with bpp.
      subroutine pressureIsm2dOrder2r( nd,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,mask,rsxy,xy,um,u,un,f,gridIndexRange,dimRange,isPeriodic,boundaryCondition,ipar,rpar,ierr )
     !======================================================================
...
! define interfaceType values for fortran routines
     ...
     !...........end   statement functions
      ! write(*,*) 'Inside pressureIsm2dOrder2r...'
      ...
      if( dt>0. )then
        ...
        write(*,'("### pressureIsm2dOrder2r : ERROR: dt=",1pe12.4," is not greater than zero!")') dt
        stop 6666
      end if
       ...
       ! -- evaluate the divergence damping ---
         ...
         if( mask(i1,i2,i3).gt.0 )then
           f(i1,i2,i3)=cdvOverDtSq*(unx22r(i1,i2,i3,u1c)-ux22r(i1,i2,i3,u1c)+uny22r(i1,i2,i3,u2c)-uy22r(i1,i2,i3,u2c))
         end if
         end do
         end do
         end do
        ...
        do side=0,1
         ...
         if( boundaryCondition(side,axis) == dirichletBoundaryCondition )then
           ...
           !      vy = 0 
               ...
               if( mask(i1,i2,i3).ne.0 )then
                 if( axis==0 )then
                   f(i1,i2,i3) = 2.*mu*unx22r(i1,i2,i3,u1c)
                 else if( axis==1 )then
                   f(i1,i2,i3) = 2.*mu*uny22r(i1,i2,i3,u2c)
                 else
                 end if 
                 if( twilightZone.eq.1 )then
                     call ogDeriv(ep,0,0,0,0,xy(i1,i2,i3,0),xy(i1,i2,i3,1),0.,t,pc,pe)
               ...
               if( gridType.eq.rectangular )then
                 if( axis==0 ) then
                   ...
                   if( useCurlCurlBoundaryCondition.eq.1 )then
                       f(j1,j2,j3) =   is*mu*unxy22r(i1,i2,i3,u2c)
                   else
                     f(j1,j2,j3) =  -is*mu*unxx22r(i1,i2,i3,u1c)
                   end if
                   ! write(*,'("pressure BC: i1,i2=",2i3," is=",i2," vxy =",1pe12.4," uxx=",1pe12.4)') i1,i2,is,unxy22r(i1,i2,i3,u2c),unxx22r(i1,i2,i3,u1c)
                 else if( axis==1 )then
                   if( useCurlCurlBoundaryCondition.eq.1 )then
                       f(j1,j2,j3) =   is*mu*unxy22r(i1,i2,i3,u1c)
                   else
                     f(j1,j2,j3) =  -is*mu*unyy22r(i1,i2,i3,u2c)
                   end if
                 else
                    ! axis == 2 
                 end if
                 ...
                 if( useCurlCurlBoundaryCondition.eq.1 )then
                   ! Use curl-curl form of the BC:  NOTE: USE un
                     f(j1,j2,j3) = mu*( an1*( -unxy22r(i1,i2,i3,u2c) + unyy22r(i1,i2,i3,u1c) ) + an2*( unxx22r(i1,i2,i3,u2c) - unxy22r(i1,i2,i3,u1c) ) )
                 else
                   !  p.n = n.( mu Delta( uv ) )
                     f(j1,j2,j3) = mu*( an1*(  unxx22r(i1,i2,i3,u1c) + unyy22r(i1,i2,i3,u1c) ) + an2*( unxx22r(i1,i2,i3,u2c) + unyy22r(i1,i2,i3,u2c) ) )
                 end if
           ...
           write(*,'("pressureIsm2dOrder2r:fill RHS for pressure equation, unexpected boundaryCondition=",i4)') boundaryCondition(side,axis)
             stop 6666
         end if
        end do ! end side
        end do ! end axis
