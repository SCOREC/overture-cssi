! This file automatically generated from pressureIsmOpt.bf90 with bpp.
      subroutine pressureIsm3dOrder2c( nd,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,mask,rsxy,xy,um,u,un,f,gridIndexRange,dimRange,isPeriodic,boundaryCondition,ipar,rpar,ierr )
     !======================================================================
...
! define interfaceType values for fortran routines
     ...
     !...........end   statement functions
      ! write(*,*) 'Inside pressureIsm3dOrder2c...'
      ...
      ! c2dtsq=c2*dtsq
       checkForNans=.false.
      if( t<=2*dt )then
        write(*,'("## pressureIsm3dOrder2c: dt=",1pe12.4," u1c,u2c,u3c,pc=",4i3," t=",1pe12.3)') dt,u1c,u2c,u3c,pc,t
      ...
      if( dt>0. )then
        ...
        write(*,'("### pressureIsm3dOrder2c : ERROR: dt=",1pe12.4," is not greater than zero!")') dt
        stop 6666
      end if
       ...
       ! -- evaluate the divergence damping ---
         ...
         if( mask(i1,i2,i3).gt.0 )then
           f(i1,i2,i3)=cdvOverDtSq*(unx23(i1,i2,i3,u1c)-ux23(i1,i2,i3,u1c)+uny23(i1,i2,i3,u2c)-uy23(i1,i2,i3,u2c)+unz23(i1,i2,i3,u3c)-uz23(i1,i2,i3,u3c))
         end if
         end do
         end do
         end do
       ...
       ! ---------- BEGIN LOOP OVER BOUNDARIES -------
        ...
        if( boundaryCondition(0,0).lt.0 )then
          ...
          extra1a=numGhost0  ! include interpolation points since we assign ghost points outside these
        end if
        ...
        if( boundaryCondition(0,1).lt.0 )then
          ...
          extra2a=numGhost0  ! include interpolation points since we assign ghost points outside these
        end if
        ! **NOTE** the bc on the right may be negative even it is not on the left (for parallel)
        ...
        if(  nd.eq.3 )then
         if( boundaryCondition(0,2).lt.0 )then
           ...
           extra3a=numGhost0  ! include interpolation points since we assign ghost points outside these
         end if
         ! **NOTE** the bc on the right may be negative even it is not on the left (for parallel)
        ...
        do side=0,1
         ...
         if( boundaryCondition(side,axis) == dirichletBoundaryCondition )then
             ...
             ! write(*,'("pressure-RHS curvilinear TRACTION CBC")') 
               ...
               if( mask(i1,i2,i3).ne.0 )then
                 ...
                 ! #End              
                   ...
                   !                 [ n1*vx+n2*vy+n3*vz ] 
                   !                 [ n1*wx+n2*wy+n3*wz ] 
                   f(i1,i2,i3) = 2.*mu*( an1*( an1*unx23(i1,i2,i3,u1c) + an2*uny23(i1,i2,i3,u1c) + an3*unz23(i1,i2,i3,u1c) ) + an2*( an1*unx23(i1,i2,i3,u2c) + an2*uny23(i1,i2,i3,u2c) + an3*unz23(i1,i2,i3,u2c) ) + an3*( an1*unx23(i1,i2,i3,u3c) + an2*uny23(i1,i2,i3,u3c) + an3*unz23(i1,i2,i3,u3c) ) ) 
                   if( isnan( f(i1,i2,i3) ) )then
                     write(*,'("pRHS: Traction BC: nan found, grid=",i3," i1,i2,i3=",3i3)') grid,i1,i2,i3 
                     write(*,'("    : an1,an2,an3=",3(1pe12.4,1x))') an1,an2,an3
                     write(*,'("    : u1x,u1y,u1z=",3(1pe12.4,1x))') unx23(i1,i2,i3,u1c),uny23(i1,i2,i3,u1c),unz23(i1,i2,i3,u1c)
                     write(*,'("    : u2x,u2y,u2z=",3(1pe12.4,1x))') unx23(i1,i2,i3,u2c),uny23(i1,i2,i3,u2c),unz23(i1,i2,i3,u2c)
                     write(*,'("    : u3x,u3y,u3z=",3(1pe12.4,1x))') unx23(i1,i2,i3,u3c),uny23(i1,i2,i3,u3c),unz23(i1,i2,i3,u3c)
                   end if                                  
                 if( twilightZone.eq.1 )then
                     call ogDeriv(ep,0,0,0,0,xy(i1,i2,i3,0),xy(i1,i2,i3,1),xy(i1,i2,i3,2),t,pc,pe)
                     ...
                     f(i1,i2,i3) = f(i1,i2,i3) + pe - 2.*mu*( an1*( an1*uex + an2*uey + an3*uez ) + an2*( an1*vex + an2*vey + an3*vez ) + an3*( an1*wex + an2*wey + an3*wez ) )                                
                 end if 
               ...
               if( gridType.eq.rectangular )then
                 if( axis==0 ) then
                   ...
                   if( useCurlCurlBoundaryCondition.eq.1 )then
                       f(j1,j2,j3) =   is*mu*( unxy23(i1,i2,i3,u2c) + unxz23(i1,i2,i3,u3c) )
                   else
                     f(j1,j2,j3) =  -is*mu*unxx23(i1,i2,i3,u1c)
                   end if
                   ! write(*,'("pressure BC: i1,i2=",2i3," is=",i2," vxy =",1pe12.4," uxx=",1pe12.4)') i1,i2,is,unxy22r(i1,i2,i3,u2c),unxx22r(i1,i2,i3,u1c)
                 else if( axis==1 )then
                   if( useCurlCurlBoundaryCondition.eq.1 )then
                       f(j1,j2,j3) =   is*mu*( unxy23(i1,i2,i3,u1c) + unyz23(i1,i2,i3,u3c) )
                   else
                     f(j1,j2,j3) =  -is*mu*unyy23(i1,i2,i3,u2c)
                   end if
                 else
                    ! axis == 2 
                     if( useCurlCurlBoundaryCondition.eq.1 )then
                       f(j1,j2,j3) =   is*mu*( unxz23(i1,i2,i3,u1c) + unyz23(i1,i2,i3,u2c) )
                     else
                       f(j1,j2,j3) =  -is*mu*unzz23(i1,i2,i3,u3c)
                     end if
                 end if
                 ...
                 if( useCurlCurlBoundaryCondition.eq.1 )then
                   ! Use curl-curl form of the BC:  NOTE: USE un
                     f(j1,j2,j3) = mu*( an1*(                       unyy23(i1,i2,i3,u1c) + unzz23(i1,i2,i3,u1c) - unxy23(i1,i2,i3,u2c) - unxz23(i1,i2,i3,u3c) ) + an2*(  unxx23(i1,i2,i3,u2c)                      + unzz23(i1,i2,i3,u2c) - unxy23(i1,i2,i3,u1c) - unyz23(i1,i2,i3,u3c) ) + an3*(  unxx23(i1,i2,i3,u3c) + unyy23(i1,i2,i3,u3c)                      - unxz23(i1,i2,i3,u1c) - unyz23(i1,i2,i3,u2c) )  )
                 else
                   !  p.n = n.( mu Delta( uv ) )
                     f(j1,j2,j3) = mu*( an1*( unxx23(i1,i2,i3,u1c) + unyy23(i1,i2,i3,u1c) + unzz23(i1,i2,i3,u1c) ) + an2*( unxx23(i1,i2,i3,u2c) + unyy23(i1,i2,i3,u2c) + unzz23(i1,i2,i3,u2c) ) + an3*( unxx23(i1,i2,i3,u3c) + unyy23(i1,i2,i3,u3c) + unzz23(i1,i2,i3,u3c) ) )
                 end if
           ...
           write(*,'("pressureIsm3dOrder2c:fill RHS for pressure equation, unexpected boundaryCondition=",i4)') boundaryCondition(side,axis)
             stop 6666
         end if
        end do ! end side
        end do ! end axis
     ...
     !
     !      #If "3" eq "2"
     !        write(*,*) 'Inside pressureIsm: order=2 dim=3'
