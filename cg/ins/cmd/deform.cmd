*
* cgins example: test deforming grids (flow past a deforming disk)
*
* Usage:
*   
...
* 
*  cgins deform -g=circleDeform -dg=ice -nu=.05 -tf=2. -tp=.02 -model=ins -go=halt 
*  cgins deform -g=circleDeform -dg=ice -nu=.05 -tf=2. -tp=.02 -model=boussinesq -go=halt 
* 
* -- deforming ellipse: 
*  cgins deform -g=diskDeformoutere1.hdf -bg=outerSquare -dg=outerInterface -dt="ellipse deform" -da=2. -nu=.1 -tf=2. -tp=.02 -model=ins -go=halt 
* 
* -- deforming circle with 2 grids on each side of the interface
* cgins deform -g=diskDeformSplitoutere2 -bg=outerSquare -dg="outerInterface1\n outerInterface2" -dt="ellipse deform" -da=2. -nu=.1 -tf=2. -tp=.02 -model=ins -go=halt
* 
* -- deforming SPHERE
* cgins noplot deform -g=sibDeforme2.order2 -bg=box -dg="share=100" -dt="sphere deform" -da=1. -nu=.05 -tf=1. -tp=.1 -model=ins -solver=best -go=go -go=go -show=sibDeform2.show >! sibDeform2.out
* 
* cgins noplot deform -g=sibDeforme4.order2 -bg=box -dg="share=100" -dt="sphere deform" -da=1. -nu=.01 -tf=2. -tp=.2 -model=ins -solver=best -df=.5 -go=go -show=sibDeform.show >! sibDeform4.out
* 
* -- advect
*  cgins deform -g=diskDeformoutere1 -bcn=slipWall -bg=outerSquare -dg=outerInterface -dt="advect body" -nu=.1 -tf=2. -tp=.02 -model=ins -go=halt
* 
* -- elasticShell -- simple spring model (not working so well yet)
*  cgins deform -g=diskDeformoutere1 -bg=outerSquare -dg=outerInterface -dt="elastic shell" -nu=.1 -tf=20. -tp=.1 -model=ins -go=halt
*  cgins deform -g=diskDeformoutere2 -bg=outerSquare -dg=outerInterface -dt="elastic shell" -nu=.02 -tf=20. -tp=.2 -model=ins -rhoe=10. -ke=1. -be=0.1 -ad2e=5. -go=halt
* 
* -- userDeformingSurface
*  cgins deform -g=diskDeformoutere1 -bcn=slipWall -bg=outerSquare -dg=outerInterface -dt="user defined deforming surface" -nu=.1 -tf=.4 -tp=.02 -model=ins -go=halt
* 
