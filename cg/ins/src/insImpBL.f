! This file automatically generated from insImpBL.bf with bpp.
! **************************************************************************************************
!   Define the full implicit matrix for a Zero Equation Model (e.g. Baldwin Lomax) 
...
! ==============================================================================================================
!   Fill in the coefficients for the Baldwin-Lomax model : Incompressible Navier-Stokes Equations
! 
!  See also : lineSolveBL.h 
! ==============================================================================================================
...
! #beginMacro defineDivNuGradu3d(NAME,au,av,aw)
! #defineMacro NAME(u)!      ( au ## 11ph*(u(i1+1,i2,i3,uc)-u(i1,i2,i3,uc)) - au ## 11mh*(u(i1,i2,i3,uc)-u(i1-1,i2,i3,uc)) )!    + ( au ## 22ph*(u(i1,i2+1,i3,uc)-u(i1,i2,i3,uc)) - au ## 22mh*(u(i1,i2,i3,uc)-u(i1,i2-1,i3,uc)) )!    + ( au ## 33ph*(u(i1,i2,i3+1,uc)-u(i1,i2,i3,uc)) - au ## 33mh*(u(i1,i2,i3,uc)-u(i1,i2,i3-1,uc)) )!    + (au ## 12pzz*(u(i1+1,i2+1,i3,uc)-u(i1+1,i2-1,i3,uc))-au ## 12mzz*(u(i1-1,i2+1,i3,uc)-u(i1-1,i2-1,i3,uc)))!    + (au ## 13pzz*(u(i1+1,i2,i3+1,uc)-u(i1+1,i2,i3-1,uc))-au ## 13mzz*(u(i1-1,i2,i3+1,uc)-u(i1-1,i2,i3-1,uc)))!    + (au ## 21zpz*(u(i1+1,i2+1,i3,uc)-u(i1-1,i2+1,i3,uc))-au ## 21zmz*(u(i1+1,i2-1,i3,uc)-u(i1-1,i2-1,i3,uc)))!    + (au ## 23zpz*(u(i1,i2+1,i3+1,uc)-u(i1,i2+1,i3-1,uc))-au ## 23zmz*(u(i1,i2-1,i3+1,uc)-u(i1,i2-1,i3-1,uc)))!    + (au ## 31zzp*(u(i1+1,i2,i3+1,uc)-u(i1-1,i2,i3+1,uc))-au ## 31zzm*(u(i1+1,i2,i3-1,uc)-u(i1-1,i2,i3-1,uc)))!    + (au ## 32zzp*(u(i1,i2+1,i3+1,uc)-u(i1,i2-1,i3+1,uc))-au ## 32zzm*(u(i1,i2+1,i3-1,uc)-u(i1,i2-1,i3-1,uc))) 
! #endMacro

! The ## does not work in the next macro, use a beginMacro to define this macro
...
! ******************* Here we now define the subroutine insImpBL ****************************
      subroutine insImpBL(nd,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
...
!======================================================================
     ...
     & checkForInflowAtOutFlow)
      if( ok.eq.0 )then
        write(*,'("*** insImpBL:ERROR: checkForInflowAtOutFlow NOT 
     & FOUND")')
      else
        if( debug.gt.4 )then
         write(*,'("*** insImpBL:checkForInflowAtOutFlow=",i6)') 
     & checkForInflowAtOutFlow
        end if
      end if
      ok = getInt(pdb,'outflowOption',outflowOption)
      if( ok.eq.0 )then
        write(*,'("*** insImpBL:ERROR: outflowOption NOT FOUND")')
      else
        if( debug.gt.4 )then
         write(*,'("*** insImpBL:outflowOption=",i6)') outflowOption
        end if
      end if
      ok = getReal(pdb,'advectionCoefficient',advectionCoefficient)
      if( ok.eq.0 )then
        write(*,'("*** insImpBL:ERROR: advectionCoefficient NOT FOUND")
     & ')
      else
        if( debug.gt.4 )then
         write(*,'("*** insImpBL:advectionCoefficient=",f5.2)') 
     & advectionCoefficient
        end if
      end if
      ok = getInt(pdb,'vsc',vsc)
      if( ok.eq.0 )then
        write(*,'("*** insImpBL:ERROR: vsc NOT FOUND")')
      end if
     ...
     & fillCoefficientsScalarSystem)
      if( ok.eq.0 )then
        write(*,'("*** insImpBL:ERROR: fillCoefficientsScalarSystem 
     & NOT FOUND")')
      else
        if( debug.gt.4 )then
         write(*,'("*** insImpBL:fillCoefficientsScalarSystem=",i6)') 
     & fillCoefficientsScalarSystem
        end if
      end if
      if( orderOfAccuracy.ne.2 .and. orderOfAccuracy.ne.4 )then
        write(*,'("insImpBL:ERROR orderOfAccuracy=",i6)') 
     & orderOfAccuracy
        stop 1
      end if
      if( gridType.ne.rectangular .and. gridType.ne.curvilinear )then
        write(*,'("insImpBL:ERROR gridType=",i6)') gridType
        stop 2
      end if
      if( uc.lt.0 .or. vc.lt.0 .or. (nd.eq.3 .and. wc.lt.0) )then
        write(*,'("insImpBL:ERROR uc,vc,ws=",3i6)') uc,vc,wc
        stop 4
      end if
!      write(*,'("insImpBL: turbulenceModel=",2i6)') turbulenceModel
!      write(*,'("insImpBL: nd,uc,vc,wc,kc=",2i6)') nd,uc,vc,wc,kc
      if( turbulenceModel.eq.kEpsilon .and. (kc.lt.uc+nd .or. 
     & kc.gt.1000) )then
        write(*,'("insImpBL:ERROR in kc: nd,uc,vc,wc,kc=",2i6)') nd,uc,
     & vc,wc,kc
        stop 5
      end if
! *****************************************************************************************************8
     ...
     & halfWidth))
      ...
      if( use2ndOrderAD.ne.0 )then
        if( debug .gt.3 )then
          write(*,'(" insImpBL: INFO: 2nd order art-diss is on: ad21,
     & ad22=",2(e9.2,1x))') ad21,ad22
      ...
      if( pdeModel.eq.BoussinesqModel )then
        if( debug .gt.3 )then
          write(*,'(" insImpBL: INFO: Boussinesq: kThermal,
     ...
     & fillCoefficientsScalarSystem.le.fillCoeffW )then
      ...
      if( implicitVariation.eq.implicitViscous )then
        ...
        write(*,'(" insImpBL: ERROR: unexpected implicitVariation=",i6)
     & ') implicitVariation
        stop 7200
      end if
      ...
      if( debug.gt.3 )then
        if( evalRightHandSide.eq.1 )then
          write(*,'("insImpBL: *EVAL RHS : implicitOption=",i2," (
     & 0=all,1=none,2=sep,3=all)")')  implicitOption
        endif
      endif
      if( debug.gt.7 )then
        if( evalRightHandSide.eq.1 )then
          write(*,'("insImpBL: ******** EVAL RHS **********")')
        end if
        if( evalResidual.eq.1 )then
          write(*,'("insImpBL: ******** EVAL RESIDUAL **********")')
        end if
        if( fillCoefficients.eq.1 )then
          write(*,'("insImpBL: ******** FILL COEFF **********")')
        end if
        ! ' 
        write(*,'("insImpBL: pdeModel,nonLinear,impVar=",3i4)') 
     & pdeModel,nonlinearTermsAreImplicit,implicitVariation
        if( fillCoefficients.eq.1 )then
          write(*,'("insImpBL: stencilSize,ncc=",2i4)') stencilSize,ncc
        end if
        write(*,'("insImpBL: aDt,bDt,nuDt=",3e10.2)') aDt,bDt,nuDt
        write(*,'("insImpBL: implicitFactor,nuImp,aImp=",f4.2,1x,
...
!   $STENCIL_WIDTH : stencil width of the local coeff-matrix (such as xCoeff, yCoeff, lapCoeff, ...)
     ...
     & bv22ph -bv22mh
          ...
          ! Now add on the identity and nonlinear terms, and artificial dissipation (which is always fully implicit)
     ...
     & i2,i3+1,wc)-6.*u(i1,i2,i3,wc))
        ...
        write(*,'("insImpBL: ERROR - not implemented for 
