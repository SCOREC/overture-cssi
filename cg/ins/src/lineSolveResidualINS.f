! This file automatically generated from insLineSolveNew.bf with bpp.
      subroutine lineSolveResidualINS(nd,
...
c         cwk=1
      ...
      if( orderOfAccuracy.eq.2 )then
        if( gridType.eq.rectangular )then
          ...
          if( nd.eq.2 )then
           ...
           ! * defineDerivativeMacros(DIM,2,rectangular)
            do i3=n3a,n3b
            do i2=n2a,n2b
            do i1=n1a,n1b
     ...
     & adSelfAdjoint2dR(i1,i2,i3,vc)
              ...
              ! compute adc2, adc4: 
               ...
               adc2= ad21 + cd22*adc
               adc4= ad41 + cd42*adc
              residual(i1,i2,i3,uc)=residual(i1,i2,i3,uc)+ad2(adc2,uc)+
     & ad4(adc4,uc)
              residual(i1,i2,i3,vc)=residual(i1,i2,i3,vc)+ad2(adc2,vc)+
     & ad4(adc4,vc)
             end if
     ...
     & tc)+ad4(adc4,tc)
           ...
           if( mask(i1,i2,i3).gt.0 )then
             ...
             residual(i1,i2,i3,uc)=f(i1,i2,i3,uc)-u(i1,i2,i3,uc)*ux2(
     & uc)-u(i1,i2,i3,vc)*uy2(uc)-u(i1,i2,i3,wc)*uz2(uc)-ux2(pc)+nu*
     & lap3d2(uc)-thermalExpansivity*gravity(0)*u(i1,i2,i3,tc)
             residual(i1,i2,i3,vc)=f(i1,i2,i3,vc)-u(i1,i2,i3,uc)*ux2(
     & vc)-u(i1,i2,i3,vc)*uy2(vc)-u(i1,i2,i3,wc)*uz2(vc)-uy2(pc)+nu*
     & lap3d2(vc)-thermalExpansivity*gravity(1)*u(i1,i2,i3,tc)
             residual(i1,i2,i3,wc)=f(i1,i2,i3,wc)-u(i1,i2,i3,uc)*ux2(
     & wc)-u(i1,i2,i3,vc)*uy2(wc)-u(i1,i2,i3,wc)*uz2(wc)-uz2(pc)+nu*
     & lap3d2(wc)-thermalExpansivity*gravity(2)*u(i1,i2,i3,tc)
             if(  use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then
              residual(i1,i2,i3,uc)=residual(i1,i2,i3,uc)+
     & adSelfAdjoint3dR(i1,i2,i3,uc)
              residual(i1,i2,i3,vc)=residual(i1,i2,i3,vc)+
     & adSelfAdjoint3dR(i1,i2,i3,vc)
              residual(i1,i2,i3,wc)=residual(i1,i2,i3,wc)+
     & adSelfAdjoint3dR(i1,i2,i3,wc)
             else if( use4thOrderAD.eq.1 )then
              ! compute adc2, adc4: 
               ...
               adc = abs(u0x)+abs(u0y)+abs(u0z)+abs(v0x)+abs(v0y)+abs(
     & v0z)+abs(w0x)+abs(w0y)+abs(w0z)
               adc2= ad21 + cd22*adc
               adc4= ad41 + cd42*adc
              residual(i1,i2,i3,uc)=residual(i1,i2,i3,uc)+ad23(adc2,uc)
     & +ad43(adc4,uc)
              residual(i1,i2,i3,vc)=residual(i1,i2,i3,vc)+ad23(adc2,vc)
     & +ad43(adc4,vc)
              residual(i1,i2,i3,wc)=residual(i1,i2,i3,wc)+ad23(adc2,vc)
     & +ad43(adc4,wc)
             end if
             if( computeTemperature.ne.0 )then
              residual(i1,i2,i3,tc)=f(i1,i2,i3,tc)-u(i1,i2,i3,uc)*ux2(
     & tc)-u(i1,i2,i3,vc)*uy2(tc)-u(i1,i2,i3,wc)*uz2(tc)+kThermal*
     & lap3d2(tc)
               ! --- artificial dissipation for T: do this for now: 
               if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then
                 residual(i1,i2,i3,tc)=residual(i1,i2,i3,tc)+
     & adSelfAdjoint3dR(i1,i2,i3,tc)
               else if( use4thOrderAD.eq.1 )then
                residual(i1,i2,i3,tc)=residual(i1,i2,i3,tc)+ad23(adc2,
     & tc)+ad43(adc4,tc)
              end if
             end if
           end if
          ...
          if( nd.eq.2 )then
           ...
           ! * defineDerivativeMacros(DIM,2,curvilinear)
            do i3=n3a,n3b
            do i2=n2a,n2b
            do i1=n1a,n1b
           if( mask(i1,i2,i3).gt.0 )then
     ...
     & uc)-u(i1,i2,i3,vc)*uy2c(uc)-ux2c(pc)+nu*lap2d2c(uc)-
     & thermalExpansivity*gravity(0)*u(i1,i2,i3,tc)
             residual(i1,i2,i3,vc)=f(i1,i2,i3,vc)-u(i1,i2,i3,uc)*ux2c(
     & vc)-u(i1,i2,i3,vc)*uy2c(vc)-uy2c(pc)+nu*lap2d2c(vc)-
     & thermalExpansivity*gravity(1)*u(i1,i2,i3,tc)
             if(  use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then
              residual(i1,i2,i3,uc)=residual(i1,i2,i3,uc)+
     ...
     & tc)+ad4(adc4,tc)
           ...
           if( mask(i1,i2,i3).gt.0 )then
             ...
             residual(i1,i2,i3,uc)=f(i1,i2,i3,uc)-u(i1,i2,i3,uc)*ux3c(
     & uc)-u(i1,i2,i3,vc)*uy3c(uc)-u(i1,i2,i3,wc)*uz3c(uc)-ux3c(pc)+
     & nu*lap3d2c(uc)                                 -
     & thermalExpansivity*gravity(0)*u(i1,i2,i3,tc)
             residual(i1,i2,i3,vc)=f(i1,i2,i3,vc)-u(i1,i2,i3,uc)*ux3c(
     & vc)-u(i1,i2,i3,vc)*uy3c(vc)-u(i1,i2,i3,wc)*uz3c(vc)-uy3c(pc)+
     & nu*lap3d2c(vc)-thermalExpansivity*gravity(1)*u(i1,i2,i3,tc)
             residual(i1,i2,i3,wc)=f(i1,i2,i3,wc)-u(i1,i2,i3,uc)*ux3c(
     & wc)-u(i1,i2,i3,vc)*uy3c(wc)-u(i1,i2,i3,wc)*uz3c(wc)-uz3c(pc)+
     & nu*lap3d2c(wc)-thermalExpansivity*gravity(2)*u(i1,i2,i3,tc)
             if(  use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then
              residual(i1,i2,i3,uc)=residual(i1,i2,i3,uc)+
     & adSelfAdjoint3dC(i1,i2,i3,uc)
              residual(i1,i2,i3,vc)=residual(i1,i2,i3,vc)+
     & adSelfAdjoint3dC(i1,i2,i3,vc)
              residual(i1,i2,i3,wc)=residual(i1,i2,i3,wc)+
     & adSelfAdjoint3dC(i1,i2,i3,wc)
             else if( use4thOrderAD.eq.1 )then
              ! compute adc2, adc4: 
     ...
     & +ad43(adc4,wc)
             ...
             if( computeTemperature.ne.0 )then
              residual(i1,i2,i3,tc)=f(i1,i2,i3,tc)-u(i1,i2,i3,uc)*ux3c(
     & tc)-u(i1,i2,i3,vc)*uy3c(tc)-u(i1,i2,i3,wc)*uz3c(tc)+kThermal*
     & lap3d2c(tc)
               ! --- artificial dissipation for T: do this for now: 
               if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then
                 residual(i1,i2,i3,tc)=residual(i1,i2,i3,tc)+
     & adSelfAdjoint3dC(i1,i2,i3,tc)
               else if( use4thOrderAD.eq.1 )then
                residual(i1,i2,i3,tc)=residual(i1,i2,i3,tc)+ad23(adc2,
     & tc)+ad43(adc4,tc)
              end if
