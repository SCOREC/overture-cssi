! This file automatically generated from interfaceOpt3dOrder4.bf90 with bpp.
 subroutine interfaceMxMLA3dOrder4c( nd, nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,gridIndexRange1, u1,u1n,u1m, wk1,mask1,rsxy1, xy1, p1,p1n,p1m, q1,q1n,q1m, boundaryCondition1, md1a,md1b,md2a,md2b,md3a,md3b,gridIndexRange2, u2,u2n,u2m, wk2,mask2,rsxy2, xy2, p2,p2n,p2m, q2,q2n,q2m, boundaryCondition2, ipar, rpar, aa2,aa4,aa8, ipvt2,ipvt4,ipvt8, ierr )
 ! ===================================================================================
         ...
         numberOfPolarizationVectors2=0
             ...
             if( numParallelGhost.eq.3 )then
               ...
               if( orderOfAccuracy.eq.4 .and. numParallelGhost.gt.0 )then
                 if( t.le. 1.5*dt .and. debug.gt.0 )then
                   write(*,'(/,"---------------------------------------------------------------")')
                   write(*,'("interface3dOrder4:WARNING: orderOfAccuracy=",i2," but numParallelGhost=",i3)') orderOfAccuracy,numParallelGhost
                   write(*,'("interface3dOrder4: Choose numParallelGhost==3 to make answers match to np=1 results")') 
                   write(*,'("---------------------------------------------------------------",/)')
                 end if 
               end if
             end if 
             ...
             if( axis1.ne.0 )then
               ...
               if( boundaryCondition1(0,0).le.0 )then ! parallel ghost may only have bc<0 on one side
                 ...
                 if( boundaryCondition2(0,0).gt.0 )then
                   write(*,'("interface3dOrder4: bc is inconsistent")')
                   stop 178
                 end if
               end if
             ...
             if( orderOfAccuracy.eq.2 .and. orderOfExtrapolation.lt.3 )then
               write(debugFile,'(" ERROR: interface3dOrder4: orderOfExtrapolation<3 ")')
               stop 7716
             end if
             if( orderOfAccuracy.eq.4 .and. orderOfExtrapolation.lt.4 )then
               write(debugFile,'(" ERROR: interface3dOrder4: orderOfExtrapolation<4 ")')
               stop 7716
             end if
          ...
          ! NOTE: assign ghost points outside the ends
            if( .true. )then
              ...
              do i2=nn2a,nn2b
               j1=mm1a
               do i1=nn1a,nn1b
               u1(i1-is1,i2-is2,i3-is3,ex)=(5.*u1(i1,i2,i3,ex)-10.*u1(i1+is1,i2+is2,i3+is3,ex)+10.*u1(i1+2*is1,i2+2*is2,i3+2*is3,ex)-5.*u1(i1+3*is1,i2+3*is2,i3+3*is3,ex)+u1(i1+4*is1,i2+4*is2,i3+4*is3,ex))
               u1(i1-is1,i2-is2,i3-is3,ey)=(5.*u1(i1,i2,i3,ey)-10.*u1(i1+is1,i2+is2,i3+is3,ey)+10.*u1(i1+2*is1,i2+2*is2,i3+2*is3,ey)-5.*u1(i1+3*is1,i2+3*is2,i3+3*is3,ey)+u1(i1+4*is1,i2+4*is2,i3+4*is3,ey))
               u1(i1-is1,i2-is2,i3-is3,ez)=(5.*u1(i1,i2,i3,ez)-10.*u1(i1+is1,i2+is2,i3+is3,ez)+10.*u1(i1+2*is1,i2+2*is2,i3+2*is3,ez)-5.*u1(i1+3*is1,i2+3*is2,i3+3*is3,ez)+u1(i1+4*is1,i2+4*is2,i3+4*is3,ez))
               u2(j1-js1,j2-js2,j3-js3,ex)=(5.*u2(j1,j2,j3,ex)-10.*u2(j1+js1,j2+js2,j3+js3,ex)+10.*u2(j1+2*js1,j2+2*js2,j3+2*js3,ex)-5.*u2(j1+3*js1,j2+3*js2,j3+3*js3,ex)+u2(j1+4*js1,j2+4*js2,j3+4*js3,ex))
               u2(j1-js1,j2-js2,j3-js3,ey)=(5.*u2(j1,j2,j3,ey)-10.*u2(j1+js1,j2+js2,j3+js3,ey)+10.*u2(j1+2*js1,j2+2*js2,j3+2*js3,ey)-5.*u2(j1+3*js1,j2+3*js2,j3+3*js3,ey)+u2(j1+4*js1,j2+4*js2,j3+4*js3,ey))
               u2(j1-js1,j2-js2,j3-js3,ez)=(5.*u2(j1,j2,j3,ez)-10.*u2(j1+js1,j2+js2,j3+js3,ez)+10.*u2(j1+2*js1,j2+2*js2,j3+2*js3,ez)-5.*u2(j1+3*js1,j2+3*js2,j3+3*js3,ez)+u2(j1+4*js1,j2+4*js2,j3+4*js3,ez))
               u1(i1-2*is1,i2-2*is2,i3-2*is3,ex)=(5.*u1(i1-is1,i2-is2,i3-is3,ex)-10.*u1(i1-is1+is1,i2-is2+is2,i3-is3+is3,ex)+10.*u1(i1-is1+2*is1,i2-is2+2*is2,i3-is3+2*is3,ex)-5.*u1(i1-is1+3*is1,i2-is2+3*is2,i3-is3+3*is3,ex)+u1(i1-is1+4*is1,i2-is2+4*is2,i3-is3+4*is3,ex))
               u1(i1-2*is1,i2-2*is2,i3-2*is3,ey)=(5.*u1(i1-is1,i2-is2,i3-is3,ey)-10.*u1(i1-is1+is1,i2-is2+is2,i3-is3+is3,ey)+10.*u1(i1-is1+2*is1,i2-is2+2*is2,i3-is3+2*is3,ey)-5.*u1(i1-is1+3*is1,i2-is2+3*is2,i3-is3+3*is3,ey)+u1(i1-is1+4*is1,i2-is2+4*is2,i3-is3+4*is3,ey))
               u1(i1-2*is1,i2-2*is2,i3-2*is3,ez)=(5.*u1(i1-is1,i2-is2,i3-is3,ez)-10.*u1(i1-is1+is1,i2-is2+is2,i3-is3+is3,ez)+10.*u1(i1-is1+2*is1,i2-is2+2*is2,i3-is3+2*is3,ez)-5.*u1(i1-is1+3*is1,i2-is2+3*is2,i3-is3+3*is3,ez)+u1(i1-is1+4*is1,i2-is2+4*is2,i3-is3+4*is3,ez))
               u2(j1-2*js1,j2-2*js2,j3-2*js3,ex)=(5.*u2(j1-js1,j2-js2,j3-js3,ex)-10.*u2(j1-js1+js1,j2-js2+js2,j3-js3+js3,ex)+10.*u2(j1-js1+2*js1,j2-js2+2*js2,j3-js3+2*js3,ex)-5.*u2(j1-js1+3*js1,j2-js2+3*js2,j3-js3+3*js3,ex)+u2(j1-js1+4*js1,j2-js2+4*js2,j3-js3+4*js3,ex))
               u2(j1-2*js1,j2-2*js2,j3-2*js3,ey)=(5.*u2(j1-js1,j2-js2,j3-js3,ey)-10.*u2(j1-js1+js1,j2-js2+js2,j3-js3+js3,ey)+10.*u2(j1-js1+2*js1,j2-js2+2*js2,j3-js3+2*js3,ey)-5.*u2(j1-js1+3*js1,j2-js2+3*js2,j3-js3+3*js3,ey)+u2(j1-js1+4*js1,j2-js2+4*js2,j3-js3+4*js3,ey))
               u2(j1-2*js1,j2-2*js2,j3-2*js3,ez)=(5.*u2(j1-js1,j2-js2,j3-js3,ez)-10.*u2(j1-js1+js1,j2-js2+js2,j3-js3+js3,ez)+10.*u2(j1-js1+2*js1,j2-js2+2*js2,j3-js3+2*js3,ez)-5.*u2(j1-js1+3*js1,j2-js2+3*js2,j3-js3+3*js3,ez)+u2(j1-js1+4*js1,j2-js2+4*js2,j3-js3+4*js3,ez))
               if( dispersive.ne.noDispersion )then
                do jv=0,numberOfPolarizationVectors1-1
                  do n=0,nd-1
                    pc = n + jv*nd 
                    p1(i1  -is1,i2  -is2,i3-  is3,pc)=(5.*p1(i1,i2,i3,pc)-10.*p1(i1+is1,i2+is2,i3+is3,pc)+10.*p1(i1+2*is1,i2+2*is2,i3+2*is3,pc)-5.*p1(i1+3*is1,i2+3*is2,i3+3*is3,pc)+p1(i1+4*is1,i2+4*is2,i3+4*is3,pc))
                    p1(i1-2*is1,i2-2*is2,i3-2*is3,pc)=(5.*p1(i1-is1,i2-is2,i3-is3,pc)-10.*p1(i1-is1+is1,i2-is2+is2,i3-is3+is3,pc)+10.*p1(i1-is1+2*is1,i2-is2+2*is2,i3-is3+2*is3,pc)-5.*p1(i1-is1+3*is1,i2-is2+3*is2,i3-is3+3*is3,pc)+p1(i1-is1+4*is1,i2-is2+4*is2,i3-is3+4*is3,pc))
                  end do
                end do
                do jv=0,numberOfPolarizationVectors2-1
                  do n=0,nd-1
                    pc = n + jv*nd 
                    p2(j1  -js1,j2  -js2,j3-  js3,pc)=(5.*p2(j1,j2,j3,pc)-10.*p2(j1+js1,j2+js2,j3+js3,pc)+10.*p2(j1+2*js1,j2+2*js2,j3+2*js3,pc)-5.*p2(j1+3*js1,j2+3*js2,j3+3*js3,pc)+p2(j1+4*js1,j2+4*js2,j3+4*js3,pc))
                    p2(j1-2*js1,j2-2*js2,j3-2*js3,pc)=(5.*p2(j1-js1,j2-js2,j3-js3,pc)-10.*p2(j1-js1+js1,j2-js2+js2,j3-js3+js3,pc)+10.*p2(j1-js1+2*js1,j2-js2+2*js2,j3-js3+2*js3,pc)-5.*p2(j1-js1+3*js1,j2-js2+3*js2,j3-js3+3*js3,pc)+p2(j1-js1+4*js1,j2-js2+4*js2,j3-js3+4*js3,pc))
                  end do
                end do
               end if
                j1=j1+1
              ...
              if( mask1(i1,i2,i3).gt.0 )then
               u1(i1-is1,i2-is2,i3-is3,ex)=(4.*u1(i1,i2,i3,ex)-6.*u1(i1+is1,i2+is2,i3+is3,ex)+4.*u1(i1+2*is1,i2+2*is2,i3+2*is3,ex)-u1(i1+3*is1,i2+3*is2,i3+3*is3,ex))
               u1(i1-is1,i2-is2,i3-is3,ey)=(4.*u1(i1,i2,i3,ey)-6.*u1(i1+is1,i2+is2,i3+is3,ey)+4.*u1(i1+2*is1,i2+2*is2,i3+2*is3,ey)-u1(i1+3*is1,i2+3*is2,i3+3*is3,ey))
               u1(i1-is1,i2-is2,i3-is3,ez)=(4.*u1(i1,i2,i3,ez)-6.*u1(i1+is1,i2+is2,i3+is3,ez)+4.*u1(i1+2*is1,i2+2*is2,i3+2*is3,ez)-u1(i1+3*is1,i2+3*is2,i3+3*is3,ez))
               u2(j1-js1,j2-js2,j3-js3,ex)=(4.*u2(j1,j2,j3,ex)-6.*u2(j1+js1,j2+js2,j3+js3,ex)+4.*u2(j1+2*js1,j2+2*js2,j3+2*js3,ex)-u2(j1+3*js1,j2+3*js2,j3+3*js3,ex))
               u2(j1-js1,j2-js2,j3-js3,ey)=(4.*u2(j1,j2,j3,ey)-6.*u2(j1+js1,j2+js2,j3+js3,ey)+4.*u2(j1+2*js1,j2+2*js2,j3+2*js3,ey)-u2(j1+3*js1,j2+3*js2,j3+3*js3,ey))
               u2(j1-js1,j2-js2,j3-js3,ez)=(4.*u2(j1,j2,j3,ez)-6.*u2(j1+js1,j2+js2,j3+js3,ez)+4.*u2(j1+2*js1,j2+2*js2,j3+2*js3,ez)-u2(j1+3*js1,j2+3*js2,j3+3*js3,ez))
               u1(i1-2*is1,i2-2*is2,i3-2*is3,ex)=(4.*u1(i1-is1,i2-is2,i3-is3,ex)-6.*u1(i1-is1+is1,i2-is2+is2,i3-is3+is3,ex)+4.*u1(i1-is1+2*is1,i2-is2+2*is2,i3-is3+2*is3,ex)-u1(i1-is1+3*is1,i2-is2+3*is2,i3-is3+3*is3,ex))
               u1(i1-2*is1,i2-2*is2,i3-2*is3,ey)=(4.*u1(i1-is1,i2-is2,i3-is3,ey)-6.*u1(i1-is1+is1,i2-is2+is2,i3-is3+is3,ey)+4.*u1(i1-is1+2*is1,i2-is2+2*is2,i3-is3+2*is3,ey)-u1(i1-is1+3*is1,i2-is2+3*is2,i3-is3+3*is3,ey))
               u1(i1-2*is1,i2-2*is2,i3-2*is3,ez)=(4.*u1(i1-is1,i2-is2,i3-is3,ez)-6.*u1(i1-is1+is1,i2-is2+is2,i3-is3+is3,ez)+4.*u1(i1-is1+2*is1,i2-is2+2*is2,i3-is3+2*is3,ez)-u1(i1-is1+3*is1,i2-is2+3*is2,i3-is3+3*is3,ez))
               u2(j1-2*js1,j2-2*js2,j3-2*js3,ex)=(4.*u2(j1-js1,j2-js2,j3-js3,ex)-6.*u2(j1-js1+js1,j2-js2+js2,j3-js3+js3,ex)+4.*u2(j1-js1+2*js1,j2-js2+2*js2,j3-js3+2*js3,ex)-u2(j1-js1+3*js1,j2-js2+3*js2,j3-js3+3*js3,ex))
               u2(j1-2*js1,j2-2*js2,j3-2*js3,ey)=(4.*u2(j1-js1,j2-js2,j3-js3,ey)-6.*u2(j1-js1+js1,j2-js2+js2,j3-js3+js3,ey)+4.*u2(j1-js1+2*js1,j2-js2+2*js2,j3-js3+2*js3,ey)-u2(j1-js1+3*js1,j2-js2+3*js2,j3-js3+3*js3,ey))
               u2(j1-2*js1,j2-2*js2,j3-2*js3,ez)=(4.*u2(j1-js1,j2-js2,j3-js3,ez)-6.*u2(j1-js1+js1,j2-js2+js2,j3-js3+js3,ez)+4.*u2(j1-js1+2*js1,j2-js2+2*js2,j3-js3+2*js3,ez)-u2(j1-js1+3*js1,j2-js2+3*js2,j3-js3+3*js3,ez))
               ...
               an3=an3/aNorm
                 ...
                 ! odd symmetry for the normal component: 
                 nDotUm = an1*u2(j1-js1,j2-js2,j3-js3,ex)+an2*u2(j1-js1,j2-js2,j3-js3,ey)+an3*u2(j1-js1,j2-js2,j3-js3,ez)
                 nDotU0 = an1*u2(j1    ,j2    ,j3    ,ex)+an2*u2(j1    ,j2    ,j3    ,ey)+an3*u2(j1    ,j2    ,j3    ,ez)
                 nDotUp = an1*u2(j1+js1,j2+js2,j3+js3,ex)+an2*u2(j1+js1,j2+js2,j3+js3,ey)+an3*u2(j1+js1,j2+js2,j3+js3,ez)
                 nDotU = 2.*nDotU0-nDotUp - nDotUm
                 u2(j1-js1,j2-js2,j3-js3,ex)=u2(j1-js1,j2-js2,j3-js3,ex) +  nDotU*an1
                 u2(j1-js1,j2-js2,j3-js3,ey)=u2(j1-js1,j2-js2,j3-js3,ey) +  nDotU*an2
                 u2(j1-js1,j2-js2,j3-js3,ez)=u2(j1-js1,j2-js2,j3-js3,ez) +  nDotU*an3
                 ! first set all components to be even 
                 u1(i1-2*is1,i2-2*is2,i3-2*is3,ex)=u1(i1+2*is1,i2+2*is2,i3+2*is3,ex)
                 u1(i1-2*is1,i2-2*is2,i3-2*is3,ey)=u1(i1+2*is1,i2+2*is2,i3+2*is3,ey)
                 u1(i1-2*is1,i2-2*is2,i3-2*is3,ez)=u1(i1+2*is1,i2+2*is2,i3+2*is3,ez)
                 ! odd symmetry for the normal component: 
                 nDotUm = an1*u1(i1-2*is1,i2-2*is2,i3-2*is3,ex)+an2*u1(i1-2*is1,i2-2*is2,i3-2*is3,ey)+an3*u1(i1-2*is1,i2-2*is2,i3-2*is3,ez)
                 nDotU0 = an1*u1(i1    ,i2    ,i3    ,ex)+an2*u1(i1    ,i2    ,i3    ,ey)+an3*u1(i1    ,i2    ,i3    ,ez)
                 nDotUp = an1*u1(i1+2*is1,i2+2*is2,i3+2*is3,ex)+an2*u1(i1+2*is1,i2+2*is2,i3+2*is3,ey)+an3*u1(i1+2*is1,i2+2*is2,i3+2*is3,ez)
                 nDotU = 2.*nDotU0-nDotUp - nDotUm
                 u1(i1-2*is1,i2-2*is2,i3-2*is3,ex)=u1(i1-2*is1,i2-2*is2,i3-2*is3,ex) +  nDotU*an1
                 u1(i1-2*is1,i2-2*is2,i3-2*is3,ey)=u1(i1-2*is1,i2-2*is2,i3-2*is3,ey) +  nDotU*an2
                 u1(i1-2*is1,i2-2*is2,i3-2*is3,ez)=u1(i1-2*is1,i2-2*is2,i3-2*is3,ez) +  nDotU*an3
                 ! first set all components to be even 
                 u2(j1-2*js1,j2-2*js2,j3-2*js3,ex)=u2(j1+2*js1,j2+2*js2,j3+2*js3,ex)
                 u2(j1-2*js1,j2-2*js2,j3-2*js3,ey)=u2(j1+2*js1,j2+2*js2,j3+2*js3,ey)
                 u2(j1-2*js1,j2-2*js2,j3-2*js3,ez)=u2(j1+2*js1,j2+2*js2,j3+2*js3,ez)
                 ! odd symmetry for the normal component: 
                 nDotUm = an1*u2(j1-2*js1,j2-2*js2,j3-2*js3,ex)+an2*u2(j1-2*js1,j2-2*js2,j3-2*js3,ey)+an3*u2(j1-2*js1,j2-2*js2,j3-2*js3,ez)
                 nDotU0 = an1*u2(j1    ,j2    ,j3    ,ex)+an2*u2(j1    ,j2    ,j3    ,ey)+an3*u2(j1    ,j2    ,j3    ,ez)
                 nDotUp = an1*u2(j1+2*js1,j2+2*js2,j3+2*js3,ex)+an2*u2(j1+2*js1,j2+2*js2,j3+2*js3,ey)+an3*u2(j1+2*js1,j2+2*js2,j3+2*js3,ez)
                 nDotU = 2.*nDotU0-nDotUp - nDotUm
                 u2(j1-2*js1,j2-2*js2,j3-2*js3,ex)=u2(j1-2*js1,j2-2*js2,j3-2*js3,ex) +  nDotU*an1
                 u2(j1-2*js1,j2-2*js2,j3-2*js3,ey)=u2(j1-2*js1,j2-2*js2,j3-2*js3,ey) +  nDotU*an2
                 u2(j1-2*js1,j2-2*js2,j3-2*js3,ez)=u2(j1-2*js1,j2-2*js2,j3-2*js3,ez) +  nDotU*an3
              end if
                j1=j1+1
               end do
               j2=j2+1
