...
! =========================================================
     ...
     & resid2
      ...
      if( debug.gt.0 )then
        ...
        write(*,'(" m = q X kth= [",3(2(1pe10.2),","),"]")') real(mx),
     & imag(mx),real(my),imag(my),real(mz),imag(mz)
      end if
     ...
     & 4))
      ...
      if( debug.gt.0 )then
        Eperp = ax*qx+ay*qy+az*qz
        Epar  = ax*gx+ay*gy+az*gz
        write(*,'(" Eperp =(",1pe10.2,",",1pe10.2,")")') real(Eperp),
     & aimag(Eperp)
        write(*,'(" Epar  =(",1pe10.2,",",1pe10.2,")")') real(Epar) ,
     & aimag(Epar)

       write(*,'(" Solution: ")')

       write(*,'(" rPerp:      x1=(",1pe10.2,",",1pe10.2,")")') real(b(
     & 1)),aimag(b(1))
       write(*,'(" rParallel:  x2=(",1pe10.2,",",1pe10.2,")")') real(b(
     & 2)),aimag(b(2))
       write(*,'(" tauPerp     x3=(",1pe10.2,",",1pe10.2,")")') real(b(
     & 3)),aimag(b(3))
       write(*,'(" tauParallel x4=(",1pe10.2,",",1pe10.2,")")') real(b(
     & 4)),aimag(b(4))
     ...
     & + qDotA
        write(*,'(" check: 1st eqn: resid=",2(1pe10.2))') real(resid1),
     & imag(resid1)

        resid1 = a4s(2,1)*x1 + a4s(2,2)*x2 + a4s(2,3)*x3 + a4s(2,4)*x4 
     & + pDotA
        write(*,'(" check: 2nd eqn: resid=",2(1pe10.2))') real(resid1),
     & imag(resid1)

        resid1 = qx*( ax+er(1)-et(1) ) + qy*( ay+er(2)-et(2) ) + qz*( 
     & az+er(3)-et(3) )
        write(*,'(" check: q.( a + er - et )=",2(1pe10.2))') real(
     & resid1),imag(resid1)

        resid1 = px*( ax+er(1)-et(1) ) + py*( ay+er(2)-et(2) ) + pz*( 
     & az+er(3)-et(3) )
        write(*,'(" check: p.( a + er - et )=",2(1pe10.2))') real(
     & resid1),imag(resid1)

        resid2 = px*er(1) + py*er(2) + pz*er(3) - (pDotQ*x1 +pDotH*x2)
        write(*,'(" check: resid2=",2(1pe10.2))') real(resid2),imag(
     & resid2)
