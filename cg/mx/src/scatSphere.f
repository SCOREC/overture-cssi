...
!...............end local variables      
     ...
     & nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,xy,u,ipar,rpar )
          ...
          call scatDispersiveSphere(nd, n1a,n1b,n2a,n2b,n3a,n3b,nd1a,
     & nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,xy,u,ipar,rpar )
        end if
      ...
      if( computeMagneticField .and. (nd4b-nd4a+1).lt.12 )then
        write(*,'(" scatSphere:ERROR: not enough space to store E and 
     & H")')
      ...
      if( debug.gt.0 )then
        write(*,'(" scatSphere: k,a=",2f10.6," ex,ey,ez=",3i3)') k,a,
     & ex,ey,ez
        write(*,'("    eta0=",2(1pe12.3,1x))') real(eta0),aimag(eta0)
        write(*,'("    computeMagneticField=",l2)') 
     & computeMagneticField
        write(*,'("    computeIncidentField=",l2)') 
     & computeMagneticField
      end if
      if( ka.le.0 )then
        stop 11223
     ...
     & pn(n)
          eri=eri - pm*(2*n+1)*(     bi(n)*psin(n)  +br(n)*phin(n) )*
     & pn(n)
          ...
          if( computeMagneticField )then
            ...
            hrr=hrr + pm*(2*n+1)*( (ci-ar(n))*psin(n) +ai(n)*phin(n) )*
     & pn(n)
            hri=hri - pm*(2*n+1)*(     ai(n)*psin(n)  +ar(n)*phin(n) )*
     & pn(n)
          ...
          np1=n+1
          err=err - pm*(2*np1+1)*(     bi(np1)*psin(np1)  +br(np1)*
     & phin(np1) )*pn(np1)
     ...
     & np1) )
          ...
          if( computeMagneticField )then
            hrr=hrr - pm*(2*np1+1)*(     ai(np1) *psin(np1) +ar(np1)*
     & phin(np1) )*pn(np1)
            hri=hri - pm*(2*np1+1)*( (ci-ar(np1))*psin(np1) +ai(np1)*
     & phin(np1) )*pn(np1)

            t1 = (ci-br(np1))*psin(np1)+bi(np1)*phin(np1)
            t2 =     ai(np1)*psinp(np1)+ar(np1)*phinp(np1)
            htr=htr - pm*(2*np1+1.)/(np1*(np1+1))*( t1*pn(np1) + t2*
     & pnp(np1) )
            hpr=hpr - pm*(2*np1+1.)/(np1*(np1+1))*(-t1*pnp(np1)- t2*pn(
     & np1) )     ! note "-"

            t1 =   -(bi(np1)*psin(np1)  +br(np1)*phin(np1))
            t2 = (ci-ar(np1))*psinp(np1)+ai(np1)*phinp(np1)
            hti=hti - pm*(2*np1+1.)/(np1*(np1+1))*( t1*pn(np1) + t2*
     & pnp(np1) )
            hpi=hpi - pm*(2*np1+1.)/(np1*(np1+1))*(-t1*pnp(np1)- t2*pn(
     & np1) )     ! note "-"
     ...
     & cosPhi/kr)*(eti-eti0)),abs((sinPhi/kr)*(epi-epi0)))))
        ...
        if( computeMagneticField )then
          ...
          u(i1,i2,i3,h1+3  ) = st*cosPhi*hrr + ct*cosPhi*htr -sinPhi*
     & hpr
          u(i1,i2,i3,h2+3  ) = st*sinPhi*hrr + ct*sinPhi*htr +cosPhi*
     & hpr
          u(i1,i2,i3,h3+3  ) = ct       *hrr - st       *htr

          u(i1,i2,i3,h1+3+3) = st*cosPhi*hri + ct*cosPhi*hti -sinPhi*
     & hpi
          u(i1,i2,i3,h2+3+3) = st*sinPhi*hri + ct*sinPhi*hti +cosPhi*
     & hpi
...
!...............end local variables      
      ...
      ka=k*a
      if( debug.gt.0 )then
        write(*,'(" scatSphere: k,k/2*pi,a,am=",4f10.6," ex,ey,ez=",
     & 3i3)') k,k/twoPi,a,am,ex,ey,ez
...
!                     Ez = 0   
! =============================================================================================
      subroutine scatDispersiveSphere(nd, n1a,n1b,n2a,n2b,n3a,n3b,nd1a,
     & nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,xy,u,ipar,rpar )
...
!.............local variables
     ...
     & 0:ntermMax)
      ...
      ! *new* June 2019
      complex*16 epsHat,epsHat0,epsHat1, muHat,muHat0,muHat1, m0,m1, 
     & cHat0,cHat1, eta,eta0,eta1,etai, ss
     ...
     & 0:ntermMax)
      ...
      complex*16 jcnpka,ycnpka,psicnka,psicnpka,phicnka,phicnpka,
     & jcnpmka,ycnpmka,psicnmka,psicnpmka
...
!...............end local variables      
      ...
      computeMagneticField = ipar(12).eq.1
      if( computeMagneticField .and. (nd4b-nd4a+1).lt.12 )then
        write(*,'(" scatDispSphere:ERROR: not enough space to store E 
     & and H")')
        write(*,'(" nd4a,nd4b=",3i6)') nd4a,nd4b
        stop 4444
      end if
      ...
      if( (debug.ge.0 .and.  n1b.gt.n1a) )then
        write(*,'(" scatDispSphere: k,k/2*pi,a=",3f10.6," ex,ey,ez=",
     & 3i3)') k,k/twoPi,a,ex,ey,ez
        write(*,'(" scatDispSphere: inOut=",i2,"(1=inside), ci=",f3.1,
     & "(1=compute incident field)")') inOut,ci
        write(*,'("         computeMagneticField=",l5)') 
     & computeMagneticField
        write(*,'("         computeIncidentField=",i2)') 
     & computeIncidentField
        write(*,'(" scatDispSphere: ex,ey,ez, hx,hy,hz =",6i3)') ex,ey,
     & ez, hx,hy,hz
        write(*,'(" scatDispSphere: e1,e2,e3, h1,h2,h3 =",6i3)') e1,e2,
     & e3, h1,h2,h3
        write(*,'("   Outside    eta0 =",2e12.3)') real(eta0),imag(
     & eta0)
      ...
      do n=1,nterm-1
        ...
        if( outside )then
          an(n) = (     psicnpmka*psicnka - etai*psicnmka*psicnpka)
     & /denom1
          bn(n) = (etai*psicnpmka*psicnka -      psicnmka*psicnpka)
     & /denom2
        else
          cn(n) = -sc*ai*am/denom1  ! include factor of "m" here
          dn(n) = -sc*ai*am/denom2
     ...
     & ct2 ) ! - added  E -> H
        ...
        sMax = max(sMax,max(abs((cosPhi/betar**2)*st*(er-er0)),max(abs(
     & (cosPhi/betar)*(et-et0)),abs((sinPhi/betar)*(ep-ep0)))))

        sMax = max(sMax,max(abs((cosPhi/betar**2)*st*(hr-hr0)),max(abs(
     & (cosPhi/betar)*(ht-ht0)),abs((sinPhi/betar)*(hp-hp0)))))
 ...
 100     continue
      ...
      if( debug.gt.0 )then
        write(*,'(" >>>scatDispSphere: edge=",i1," nterm=",i3," 
     & largest final term sMax=",e10.2)') edge,nterm,sMax
        write(*,'(" >>>scatDispSphere: computeElectricField=",l1," 
     & computeMagneticField=",l1)') computeElectricField,
     & computeMagneticField
      end if
      ! '
