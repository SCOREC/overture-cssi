! This file automatically generated from scatCyl.bf with bpp.
!===================================================================================
...
!==================================================================================
      ...
      if( nz.ne.0 .or. ierr.ne.0 )then
        ...
        write(*,'("         nu,zr,zi,jr,ji=",5e12.4)') nu,zr,zi,jr(1),
     & ji(1)
      end if
...
!  
! =============================================================================================
      subroutine scatcyl(nd, n1a,n1b,n2a,n2b,n3a,n3b,nd1a,nd1b,nd2a,
...
!.............local variables
      ...
      integer i1,i2,i3,nterm,ncalc,nb,exr,eyr,hzr,exi,eyi,hzi,
     & staggeredGrid,debug
      real k,a,ka,r,theta,kr,x,y,alpha,twoPi
      real jnka(0:ntermMax),ynka(0:ntermMax),jnpka(0:ntermMax),ynpka(
     & 0:ntermMax)
      real jn(0:ntermMax),yn(0:ntermMax),jnp(0:ntermMax),ynp(
     & 0:ntermMax),an(0:ntermMax)

      real s,sr,si,srr,srt,sir,sit,rx,ry,tx,ty,pm,cnt,cnp1t,sMax,eps,
     & dir
...
!...............end local variables      
      ...
      if( option.eq.1 )then
        ! dielectric case:
        call scatDielectricCyl(nd, n1a,n1b,n2a,n2b,n3a,n3b,nd1a,nd1b,
     & nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,xy,u,ipar,rpar )
        return
      end if
      ...
      ka=k*a
      write(*,'(" scatcyl: k,a=",2f10.6," exr,eyr,hzr=",3i3," 
     & staggeredGrid=",i2)') k,a,exr,eyr,hzr,staggeredGrid
      ! ' 
      if( ka.le.0 )then
        stop 11233
      end if
      ...
      do edge=1,numEdges  ! for a staggered grid we need to evaluate along edges and the cell-centers
       ...
       do i1=n1a,n1b
         ...
         do n=1,nterm-2,2   ! nterm should be odd
           ...
           srt= srt+ pm*( -jn(n)*ynpka(n)+  yn(n)*jnpka(n))*an(n)*(-n*
     & sin(n*theta))

           si = si - pm*(  jn(n)*jnpka(n) +  yn(n)*ynpka(n) )*an(n)*cnt
           sir= sir- pm*( jnp(n)*jnpka(n) + ynp(n)*ynpka(n) )*an(n)*cnt
           sit= sit- pm*(  jn(n)*jnpka(n) +  yn(n)*ynpka(n) )*an(n)*(-
     & n*sin(n*theta))

           np1=n+1
           cnp1t=cos(np1*theta)
           sr = sr - pm*(  jn(np1)*jnpka(np1) +  yn(np1)*ynpka(np1) )*
     & an(np1)*cnp1t
           srr= srr- pm*( jnp(np1)*jnpka(np1) + ynp(np1)*ynpka(np1) )*
     & an(np1)*cnp1t
           srt= srt- pm*(  jn(np1)*jnpka(np1) +  yn(np1)*ynpka(np1) )*
     & an(np1)*(-np1*sin(np1*theta))

           si = si - pm*( -jn(np1)*ynpka(np1)+  yn(np1)*jnpka(np1))*an(
     & np1)*cnp1t
           sir= sir- pm*(-jnp(np1)*ynpka(np1)+ ynp(np1)*jnpka(np1))*an(
     & np1)*cnp1t
           sit= sit- pm*( -jn(np1)*ynpka(np1)+  yn(np1)*jnpka(np1))*an(
     & np1)*(-np1*sin(np1*theta))
           pm=-pm
         ...
         sMax = max(sMax,max(abs(sr-sr0),max(abs(srr-srr0),abs(srt-
     & srt0))))
         sMax = max(sMax,max(abs(si-si0),max(abs(sir-sir0),abs(sit-
     & sit0))))
       ...
       write(*,'(" >>>scatcyl: nterm=",i3," largest final term sMax=",
     & e10.2)') nterm,sMax
       ! '
...
! =============================================================================================
      subroutine scatDielectricCyl(nd, n1a,n1b,n2a,n2b,n3a,n3b,nd1a,
...
!.............local variables
     ...
     & dir
      ...
      real jnmka(0:ntermMax),jnpmka(0:ntermMax),ynmka(0:ntermMax),
     & ynpmka(0:ntermMax)
      real am,mka,snt,kk,rMin,aa,kkm,inc
      real mr,mi
      complex*16 ai,hnc,hnpc,detc,detic,aimn,expc,exptc,sc,scr,sct,ain,
     & expmc,expmtc,hr
      ...
      complex*16 mc,mcka,mckr,kkc,kkmc,sc0,scr0,sct0, ss,epsHat1,
     & epsHat2,eFactor,eFactorSave,eScale,eyIncident
      ! for complex Bessel:
      real zr,zi,nu,errr,erri
      real jnr(0:ntermMax),jni(0:ntermMax),ynr(0:ntermMax),yni(
     & 0:ntermMax)
      complex*16 jcn(0:ntermMax),jcnp(0:ntermMax),jcnmka(0:ntermMax),
     & jcnpmka(0:ntermMax)
...
!...............end local variables      
      ...
      write(*,'(" scatDieCyl: k,a=",2f10.6,", m=(",f10.6,",",f10.6,") 
     & exr,eyr,hzr=",3i3)') k,a,mr,mi,exr,eyr,hzr
      write(*,'(" scatDieCyl: mc*k=(",f10.6,",",f10.6,")")') real(mc*k)
     & ,aimag(mc*k)
      if( ka.le.0 )then
        stop 12233
      end if
          ...
          rMin=.75*a
           ...
           do n=0,nterm-1
             ...
             detic = 1./(am*jnmka(n)-hr*jnpmka(n))
               an(n)= (jnka(n)*jnpmka(n)-jnpka(n)*jnmka(n)*am)*
     & detic/hnpc
           ...
           do i1=n1a,n1b
             ...
             do n=1,nterm-1
               ...
               expmtc=n*cmplx(-snt,-cnt)
                 hnc=cmplx(jn(n),-yn(n))
                 hnpc=cmplx(jnp(n),-ynp(n))
                 sc = (inc*jn(n) + an(n)*hnc )*aimn*expc + (inc*jn(n) +
     &  an(n)*hnc )*aimn*expmc
                 scr= (inc*jnp(n)+ an(n)*hnpc)*aimn*expc + (inc*jnp(n)+
     &  an(n)*hnpc)*aimn*expmc
                 sct= (inc*jn(n) + an(n)*hnc )*aimn*exptc+ (inc*jn(n) +
     &  an(n)*hnc )*aimn*expmtc
             ...
             if( staggeredGrid.eq.0 )then
              ...
              ! write(*,'(" exr,exi=",2(1pe14.4)," eyr,eyi=",2(1pe14.4))')  u(i1,i2,i3,exr),u(i1,i2,i3,exi),u(i1,i2,i3,eyr),u(i1,i2,i3,eyi)
               ...
               if( edge.eq.1 )then
                ...
                u(i1,i2,i3,eyr)= dir*(rx*sir+tx*sit)/kkm
                u(i1,i2,i3,eyi)=-(rx*srr+tx*srt)/kkm
                 u(i1,i2,i3,eyr)= u(i1,i2,i3,eyr) - dir*(1.-inc)*cos(k*
     & x)
                 u(i1,i2,i3,eyi)= u(i1,i2,i3,eyi) + (1.-inc)*sin(k*x)
               else
                 ! Hz lives at the cell center 
                u(i1,i2,i3,hzr)=-dir*sr
                u(i1,i2,i3,hzi)=-si
                 u(i1,i2,i3,hzr)= u(i1,i2,i3,hzr) - dir*(1.-inc)*cos(k*
     & x)
                 u(i1,i2,i3,hzi)= u(i1,i2,i3,hzi) +     (1.-inc)*sin(k*
     & x)
               end if
             end if
           end do
           end do
           write(*,'(" >>>scatDielectricCyl: nterm=",i3," largest 
     & final term sMax=",e10.2)') nterm,sMax
           ! ' 
           end do ! edge
        else
          kk=am*k
          ...
          rMin=.75*a
           ...
           do n=0,nterm-1
             ...
             detic = 1./(mc*jcnmka(n)-hr*jcnpmka(n))
               an(n)= (jnka(n)*jcnpmka(n)-jnpka(n)*jcnmka(n)*mc)*
     & detic/hnpc
             !   write(*,'(" n=",i2," h=(",2e10.2,") hp=(",2e10.2,")  hr=(",2e10.2,") detc=",2e10.2," detic=",2e10.2," an=",2e10.2)') n,real(hnc),aimag(hnc),real(hnpc),aimag(hnpc),real(hr),aimag(hr),real(detc),aimag(detc),real(detic),aimag(detic),real(an(n)),aimag(an(n))
             ! aa = 2.*am/( am**n*(am+1./am))  ! asymptotic formula
             ! write(*,'(" n=",i2," h=(",2e10.2,") an=",2e10.2," an/asymp=",2e10.2)') n,real(hnc),aimag(hnc)! ,real(an(n)),aimag(an(n)),real(an(n)/aa),aimag(an(n)/aa)
           ...
           do i1=n1a,n1b
             ...
             ! check the size of the last terms
             sMax = max(sMax,max(cabs(sc-sc0),max(cabs(scr-scr0),cabs(
     & sct-sct0))))
             ! #If "exterior" eq "exterior"
             !   write(*,'("exterior: i1,i2=",2i3," sc=",2f9.5,", scr=",2f9.5,", sct=",2f9.5)') i1,i2,real(sc),aimag(sc),real(scr),aimag(scr),real(sct),aimag(sct)
             ! #Else
     ...
     & x)
           ...
           write(*,'(" >>>scatDieCylComplex: nterm=",i3," largest 
     & final term sMax=",e10.2)') nterm,sMax
           ! ' 
           end do ! edge
        else
     ...
     &       )
      write(*,'(" epsHat1  =",2(1pe14.4))') real(epsHat1   ),aimag(
     & epsHat1   )
      write(*,'(" epsHat2  =",2(1pe14.4))') real(epsHat2   ),aimag(
     & epsHat2   )
      ! write(*,'(" 1/eFactor=",2(1pe14.4))') real(1./eFactorSave),aimag(1./eFactorSave)
      write(*,'(" kkmc     =",2(1pe14.4))') real(kkmc      ),aimag(
     & kkmc      )
