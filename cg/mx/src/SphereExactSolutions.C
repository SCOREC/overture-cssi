// This file automatically generated from SphereExactSolutions.bC with bpp.
...
eval( real x[3], real *Ev, real *Hv /*= NULL */  )
{
  ...
  // printF("SphereExactSolutions::eval: call scatSphere x=(%g,%g,%g), rad=%g, domain=%d ...\n",x[0],x[1],x[2],rad,domain);
    ...
    for( int i=0; i<6; i++ )
    {
        Ev[i]=uLocal(0,0,0,i);        // Re(Ex), Re(Ey), Re(Ez), Im(Ex), Im(Ey), Im(Ez) 
        if( computeMagneticField )
            Hv[i]=uLocal(0,0,0,i+6);    // Re(Hx), Re(Hy), Re(Hz), Im(Hx), Im(Hy), Im(Hz) 
...
// ----------------------------------------------------------------
    ...
    // eval( xm, Ev,Hv );
    // for( int i=0; i<3; i++ ) { Em[i]=  Ev[i]+I*Ev[i+3]; } //

    // xp[0]=x[0]; xp[1]=x[1]+deps2; xp[2]=x[2];
    // eval( xp, Ev,Hv );
    // for( int i=0; i<3; i++ ) { Ep[i]=  Ev[i]+I*Ev[i+3]; } //
...
/* ----- MATLAB VERSION --------
    for ip=1:2
        ...
        % Check E wave equation:   
        %    Delta Ev = s^2 epsHat muHat Ev = - m^2 k^2 Ev = - beta^2 Ev 
        DeltaE(1:3) = Evxx(1:3) + Evyy(1:3) + Evzz(1:3)  + I*(  Evxx(4:6) + Evyy(4:6) + Evzz(4:6) ); % Complex Delta(E) 
        E(1:3) = Ev(1:3) + I*Ev(4:6);    % complex E 
        normDeltaE =  max(abs(DeltaE));
        errE = max(abs( DeltaE + beta^2*E ))/normDeltaE;

        % Check H wave equation
        DeltaH(1:3) = Hvxx(1:3) + Hvyy(1:3) + Hvzz(1:3)  + I*(  Hvxx(4:6) + Hvyy(4:6) + Hvzz(4:6) ); % Complex Delta(H) 
        H(1:3) = Hv(1:3) + I*Hv(4:6);    % complex H  
        normDeltaH =  max(abs(DeltaH));
        errH = max(abs( DeltaH + beta^2*H ))/normDeltaH;
  
        % Check E FOS equation:    eps*E_t = curl(H) 
        %  s epsHat E = curl( H )
        Hx = Hvx(1:3) + I*Hvx(4:6);    % complex H.x
        Hy = Hvy(1:3) + I*Hvy(4:6);    % complex H.y
        Hz = Hvz(1:3) + I*Hvz(4:6);    % complex H.z
        curlH(1) = Hy(3) - Hz(2);
        curlH(2) = Hz(1) - Hx(3);
        curlH(3) = Hx(2) - Hy(1);
        ...
        %     mu*H_t = -curl(E) 
        %  s muHat H = -curl( E )
        Ex = Evx(1:3) + I*Evx(4:6);    % complex E.x
        Ey = Evy(1:3) + I*Evy(4:6);    % complex E.y
        Ez = Evz(1:3) + I*Evz(4:6);    % complex E.z
        curlE(1) = Ey(3) - Ez(2);
        curlE(2) = Ez(1) - Ex(3);
        curlE(3) = Ex(2) - Ey(1);
    ...
    for ip=1:2  % check a few points 
        ...
        for side=1:2
            if side==1 
                E(1:3) = Evm(1:3) + I*Evm(4:6);    % complex E
                H(1:3) = Hvm(1:3) + I*Hvm(4:6);    % complex H
            else
                E(1:3) = Evp(1:3) + I*Evp(4:6);    % complex E
                H(1:3) = Hvp(1:3) + I*Hvp(4:6);    % complex H
            end 
