...
/// \param maxNumberOfParameters, maxNumberOfPolarizationVectors (input) : specifies the
//      max number of entries that has been allocated in the gdmPar array.
/// \param gdmParOption (input) : 1=scale a0 and a1 parameters by eps
// ================================================================================
...
#define gdmVar(m,iv) gdmPar[(m)+maxNumberOfParameters*(iv)]
    ...
    for( int iv=0; iv<dmp.numberOfPolarizationVectors; iv++ )
    {
        ...
        if( gdmParOption==1 && alphaP>0 )
        {
      // scale a0 and a1 by eps here (new way, June 14, 2018)
            const real eps=1./alphaP;  // assumes alphaP=1/eps
            gdmVar(0,iv) = gdmVar(0,iv)*eps; 
            gdmVar(1,iv) = gdmVar(1,iv)*eps;
        }
...
extern "C"
{
// Function to allow Fortran routines to eval the user defined known solution.
// Output:
//   evals[0:2] : Ex,Ey,Hz (2d) or Ex,Ey,Ez (3d)
//   pvals[0:numberOfPolarizationVectors-1] : polarization vector
//
void evalUserDefinedKnownSolution( real & t, int & grid, int & i1, int & i2, int & i3, real *evals, real *pvals, int & numberOfTimeDerivatives )
{
    assert( cgmxPointer != NULL );
    ...
    int numComponents=3;  // Ex,Ey,Hz (2d) or Ex,Ey,Ez (3d)
    RealArray ua(I1,I2,I3,numComponents), pv;
    if( numberOfPolarizationVectors>0 )
    {
        pv.redim(I1,I2,I3,numberOfDimensions*numberOfPolarizationVectors);
    }
...
#ifndef USE_PPP
  // ** FIX ME : pv,pv --> pv,qv
    cgmxPointer->getUserDefinedKnownSolution( current, t, cg, grid,ua,pv,pv,I1,I2,I3, numberOfTimeDerivatives);
#else
    ...
    for( int axis=0; axis<numberOfDimensions; axis++ )
    {
        ...
        {
            int pc = axis + jv*numberOfDimensions;
            pvals[pc]=pv(i1,i2,i3,pc);
        }
    }
       ...
       // const bool updateDissipation = addDissipation;
        ...
        // external forcings at different time levels are stored here: 
          ...
          // nonlinear model
                ...
                RealArray uDot;
                if( computeUt && addDissipation )
                {
          // We need storge for uDot  -- for now allocate a separate work space --FIX ME---
          // In some cases we could use the array for forcing if the dissipation is a separate step
                    uDot.redim(uLocal);
                    ut1ptr =uDot.getDataPointer();
                }
          ...
          // ut5ptr = xyptr;
                ...
                if( debug & 8 )
                {
                    ...
                    display(fLocal,sPrintF("fLocal BEFORE advMaxwell t=%8.2e",t),debugFile,"%8.2e ");
                    display(un,sPrintF("un BEFORE advMaxwell t=%8.2e",t),debugFile,"%8.2e ");
                }
