! This file automatically generated from getDiv.bf with bpp.
       subroutine getDiv2dOrder4r(nd,n1a,n1b,n2a,n2b,n3a,n3b,nd1a,nd1b,
...
c============================================================================================
     ...
     & )then
        ...
        if( method.ne.bamx )then
         ! --- Isotropic Maxwell ----
            if( t.lt.2*dt )then
              write(*,'("computeDiv dim=2 order=4 grid=rectangular ... 
     & t=",e10.2)') t
            end if
            ...
            ! Loop bounds -- exclude absorbing layers ? 
              ...
              do i1=n1a,n1b
               ...
               ! --- 2D -----
                   divD(i1,i2,i3) = vx42r(i1,i2,i3,ex) + vy42r(i1,i2,
     & i3,ey)
     ...
     & 1,mr)*u(i1,i2,i3,1) + K03(m,2,mr)*u(i1,i2,i3,2)
                ...
                if( t.lt.2*dt )then
                  write(*,'("computeDiv (BA) dim=2 order=4 
                ...
                ! Loop bounds -- exclude super-grid layer ? 
                  ...
                  do i1=n1a,n1b
                   ...
                   ! --- 2D -----
                     ...
                     gradDmax = max( gradDmax,abs(d1x),abs(d2x),abs(
     & d1y),abs(d2y))
                ...
                ! Loop bounds -- exclude super-grid layer ? 
                  ...
                  do i1=n1a,n1b
                   ...
                   ! --- 2D -----
                     ...
                     divDmax=max(divDmax,abs(dDiv))
                     gradDmax = max( gradDmax,abs(d1x),abs(d2x),abs(
     & d3x),abs(d1y),abs(d2y),abs(d3y))
                     bDiv = b1x + b2y
                     if( saveDivergence.eq.1 )then
                       divB(i1,i2,i3)=bDiv
                     end if
                     divBmax = max(divBmax,abs(bDiv))
                     gradBmax =  max( gradBmax,abs(b1x),abs(b2x),abs(
     & b3x),abs(b1y),abs(b2y),abs(b3y))
                    end if
                  end do
                  end do
                  end do
