! This file automatically generated from evalRadKernel.bf with bpp.
...
! ========================================================================================
      ...
      complex*16 fold3d(nd1a:nd1b,nd2a:nd2b,nd3a:nd3b,nd4a:nd4b,
     & nd5a:nd5b)
      ...
      integer numberOfModes1,numberOfModes2,orderInTime,mc,
     & currentTimeLevel,numberOfTimeLevels
      ...
      if( .false. )then
        write(*,'(" evalRadKernel: nd=",i2," mc=",i2," dt=",e10.2)') 
     & nd,mc,dt
        write(*,'(" evalRadKernel: numberOfPoles=",i3," i1a,i1b=",2i3)
     & ') numberOfPoles,i1a,i1b
        write(*,'(" evalRadKernel: n1,n2=",2i3," numberOfModes=",2i3)')
     &  n1,n2,numberOfModes1,numberOfModes2
        write(*,'(" evalRadKernel: ndzl1a:ndzl1b=",2i3)') ndzl1a,ndzl1b
        write(*,'(" evalRadKernel: numberOfTimeLevels,
     & currentTimeLevel=",2i3)') numberOfTimeLevels,currentTimeLevel
      end if
      ...
      do l=-1,orderInTime-2
        lvl(l)= mod(currentTimeLevel - l + numberOfTimeLevels, 
     & numberOfTimeLevels)
      end do
            ...
            if( abs(k2) .le. numberOfModes2 )then
              ...
              do k1fft=i1a,i1b
                ...
                if( abs(k1) .le. numberOfModes1 )then
                  ...
                  do l=0,orderInTime-2
                    level=lvl(l)
                    adon = adon + xfact*amc(l)*fold3d(level,k1fft,
     & k2fft,pole,mc)
                    xfact = xfact * expBeta
                  end do

                  adon = adon + c*dt*amc(-1)*alpha(pole)*w*w*zl3d(
     & k1fft,k2fft)
                  phi3d(k1fft,k2fft,pole,mc)=adon

                  level=lvl(-1)
                  fold3d(level,k1fft,k2fft,pole,mc)=c*dt*w*w*alpha(
     & pole)*zl3d(k1fft,k2fft)
        ...
        do k2fft=i2a,i2b
          ...
          do k1fft=i1a,i1b
            ...
            if( abs(k1)<=numberOfModes1 .and. abs(k2)<=numberOfModes2 )
     & then
              do pole=0,numberOfPoles-1
                zl3d(k1fft,k2fft) = zl3d(k1fft,k2fft) + phi3d(k1fft,
     & k2fft,pole,mc)
              end do
            end if
          end do
            ...
            if( abs(k2) .le. numberOfModes2 )then
              ...
              do k1fft=i1a,i1b
                ...
                if( abs(k1) .le. numberOfModes1 )then
                  ...
                  adon = xfact*phi3d(k1fft,k2fft,pole,mc)          ! add-on
                  do l=0,orderInTime-2
                    adon = adon + xfact*amc(l)*fold3d(l,k1fft,k2fft,
     & pole,mc)
                    xfact = xfact * expBeta
                  end do
     ...
     & k1fft,k2fft)
                  ...
                  do l=orderInTime-2,1,-1
                    fold3d(l,k1fft,k2fft,pole,mc)=fold3d(l-1,k1fft,
     & k2fft,pole,mc)
                  end do
                  fold3d(0,k1fft,k2fft,pole,mc)=c*dt*w*w*alpha(pole)*
     & zl3d(k1fft,k2fft)
