        ...
        // display(ee,"exact solution for error computation");
            ...
            // printF("getERR: BAMX: getErrTZ, numberOfPolarizationVectors=%d\n",numberOfPolarizationVectors);
                ...
                if( debug & 4 ) 
                {
                    ...
                    {
                        Range all;
                        display(errPolarization(I1,I2,I3,all),sPrintF("getErrors: errP on grid %i at t=%e",grid,t),pDebugFile,"%9.2e "); 
                        if( numberOfAtomicLevels>0 )
                        {
                            Range Na = numberOfAtomicLevels;
                            display(errNonlinear(I1,I2,I3,Na),sPrintF("getErrors: errN on grid %i at t=%e",grid,t),pDebugFile,"%9.2e "); 
                        }
                    }
                }
            }
...
//        solutionNorm(c)=getMaxValue(solutionNorm(c));
    ...
    for( int c=C.getBase(); c<=C.getBound(); c++ )
    {
        solutionNorm(c)=ParallelUtility::getMaxValue(solutionNorm(c)); // fix me -- could do all at once
        maximumError(c)=ParallelUtility::getMaxValue(maximumError(c));
    }
  ...
  // printF("getErrors: dispersionModel=%d\n",(int)dispersionModel);
    ...
    if( dispersionModel != noDispersion )
    {
      ...
      // printF("getErrors: domain=%d, numPolarizationVectors=%i\n",domain,numberOfPolarizationVectors);
            ...
            maxErrPolarization(domain) = ParallelUtility::getMaxValue(maxErrPolarization(domain)); // fix me -- could do all at once
            polarizationNorm(domain)   = ParallelUtility::getMaxValue(polarizationNorm(domain)); // fix me -- could do all at once
  ...
  // -------- PRINT ERRORS IN NONLINEAR MODELS --------
    if( nonlinearModel != noNonlinearModel )
    {
        for( int domain=0; domain<cg.numberOfDomains(); domain++ )
        {
            ...
            maxErrNonlinear(domain) = ParallelUtility::getMaxValue(maxErrNonlinear(domain)); // fix me -- could do all at once
            nonlinearNorm(domain)   = ParallelUtility::getMaxValue(nonlinearNorm(domain)); // fix me -- could do all at once
    ...
    // ** OLD WAY *** 
      ...
      // const int errorNorm = parameters.dbase.get<int >("errorNorm");
          ...
          // fPrintF(output,"%10.4e,",maximumError(c));
                    ...
                    if( dispersionModel != noDispersion )
                    {
                        fPrintF(output," P-norm by domain: [%8.2e",polarizationNorm(0));
