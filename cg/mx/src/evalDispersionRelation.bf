...
!
! =====================================================================================================
      subroutine evalEigGDM( mode, Np, c,k, a0,a1,b0,b1, sr,si, srm,sim,chir,chii, chiSumr,chiSumi )

      implicit none

      integer mode, Np
      real c,k, srm,sim
      real a0(0:*),a1(0:*),b0(0:*),b1(0:*), chir(0:*),chii(0:*), chiSumr, chiSumi
      real sr(0:*),si(0:*)
      ...
      if( Np .eq. 0 )then 
#Include "generalizedDispersionModelCompanionMatrix0.h"
...
#Include "generalizedDispersionModelCompanionMatrix4.h"
      else
        write(*,'(" evalEigGDM:ERROR: dispersion relation not implemented for Np=",i6)') Np
        stop 1234
      end if
...
!
! =====================================================================================================
      subroutine evalInverseGDM( c, sr,si, Np, a0,a1,b0,b1, kr,ki,chir,chii, chiSumr, chiSumi )
...
!-       end   
      ...
      !===========================================================================================
      subroutine evalComplexIndexOfRefraction(mu1,eps1,chi1r,chi1i, mu2,eps2,chi2r,chi2i, mr,mi)
...
! =========================================================
#beginMacro normalizeVector(qName,qx,qy,qz)
  ...
  if( aNorm > 0. )then
    ...
    write(*,'(" evalMatInterfaceSolution3d:ERROR: qName is zero!")') 
    stop 1111
  end if
#endMacro
...
! =========================================================
#beginMacro normalizeComplexVector(qName,qx,qy,qz)
  aNorm = sqrt( cabs(qx)**2+cabs(qy)**2+cabs(qz)**2)
  if( aNorm > 0. )then
    qx=qx/aNorm
    qy=qy/aNorm
    qz=qz/aNorm
    ...
    stop 1111
      ...
      ! ============================================================================================
      subroutine evalMaterialInterfaceSolution3d( sr,si, chiSum1r, chiSum1i, chiSum2r, chiSum2i, av, nv, kv,\
               eps1,eps2,mu1,mu2, \
      ...
      real eps1,eps2,mu1,mu2,sr,si,chiSum1r, chiSum1i, chiSum2r, chiSum2i
      ...
      if( debug.gt.0 )then
        ...
        write(*,'(" transmitted wave: ktn=(",1pe10.2,",",1pe10.2,") (could choose negative?)")') real(ktn),aimag(ktn)
      ...
      qDotKrCrossQ = qx*( kry*qz-krz*qy  ) + qy*( krz*qx-krx*qz ) + qz*( krx*qy-kry*qx )
      qDotKrCrossH = qx*( kry*hz-krz*hy  ) + qy*( krz*hx-krx*hz ) + qz*( krx*hy-kry*hx )
      qDotKtCrossQ = qx*( kty*qz-ktz*qy  ) + qy*( ktz*qx-ktx*qz ) + qz*( ktx*qy-kty*qx )
      qDotKtCrossM = qx*( kty*mz-ktz*my  ) + qy*( ktz*mx-ktx*mz ) + qz*( ktx*my-kty*mx )
      qDotKCrossA  = qx*( ky*az-kz*ay    ) + qy*( kz*ax-kx*az   ) + qz*( kx*ay -ky*ax  )
      ...
      pDotKrCrossQ = px*( kry*qz-krz*qy  ) + py*( krz*qx-krx*qz ) + pz*( krx*qy-kry*qx )
      pDotKrCrossH = px*( kry*hz-krz*hy  ) + py*( krz*hx-krx*hz ) + pz*( krx*hy-kry*hx )
      pDotKtCrossQ = px*( kty*qz-ktz*qy  ) + py*( ktz*qx-ktx*qz ) + pz*( ktx*qy-kty*qx )
      pDotKtCrossM = px*( kty*mz-ktz*my  ) + py*( ktz*mx-ktx*mz ) + pz*( ktx*my-kty*mx )
      pDotKCrossA  = px*( ky*az-kz*ay    ) + py*( kz*ax-kx*az   ) + pz*( kx*ay -ky*ax  )
      ...
      if( debug.gt.0 )then
       ...
       write(*,'(" a= (",2(1pe10.2),",",2(1pe10.2),",",2(1pe10.2),",",2(1pe10.2),")")') a4(4,1),a4(4,2),a4(4,3),a4(4,4)

       write(*,'(" b1=(",1pe10.2,",",1pe10.2,")")') real(b(1)),aimag(b(1))
       write(*,'(" b2=(",1pe10.2,",",1pe10.2,")")') real(b(2)),aimag(b(2))
       write(*,'(" b3=(",1pe10.2,",",1pe10.2,")")') real(b(3)),aimag(b(3))
       write(*,'(" b4=(",1pe10.2,",",1pe10.2,")")') real(b(4)),aimag(b(4))
      end if
