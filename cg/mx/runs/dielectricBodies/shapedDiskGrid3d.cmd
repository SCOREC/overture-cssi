#
#     Solid 3D shape in a channel
#
#
# usage: ogen [-noplot] shapedDiskGrid3d -factor=<num> -order=[2/4/6/8] -interp=[e/i] ...
...
# Examples:
# 
#     ogen -noplot shapedDiskGrid3d -shape=ellipse -prefix=ellipticalGrid -interp=e -factor=4
#     ogen -noplot shapedDiskGrid3d -shape=ellipse -prefix=ellipticalGrid -interp=e -factor=8
#
#     ogen -noplot shapedDiskGrid3d -shape=ellipse -prefix=ellipticalGrid -interp=e -order=4 -factor=8
# 
#  -- star-fish 
#     ogen -noplot shapedDiskGrid3d -shape=starFish -prefix=starFishGrid -interp=e -order=2 -factor=64
#     ogen -noplot shapedDiskGrid3d -shape=starFish -prefix=starFishGrid -interp=e -order=2 -factor=128
#
#  --- cross:
#   ogen -noplot shapedDiskGrid3d -shape=cross -prefix=crossGrid -interp=e -order=2 -factor=16
#   ogen -noplot shapedDiskGrid3d -shape=cross -prefix=crossGrid -interp=i -order=4 -factor=16
#
#  --- I-Beam
#   ogen -noplot shapedDiskGrid3d -shape=IBeam -prefix=IBeamGrid -interp=e -order=2 -factor=8
#   ogen -noplot shapedDiskGrid3d -shape=IBeam -prefix=IBeamGrid -interp=i -order=4 -factor=8
#
$shape="ellipse"; $ds0=.1; 
$prefix="shapedDiskGrid3d";  $rgd="var"; $angle=0.; $branch=0; $per=1; 
$order=2; $factor=1; $interp="i"; $ml=0; # default values
$orderOfAccuracy = "second order"; $ng=2; $interpType = "implicit for all grids";
$name=""; $xa=-1.; $xb=1.; $ya=-1.25; $yb=1.25; $za=-.1; $zb=.1; 
...
GetOptions( "order=i"=>\$order,"factor=f"=> \$factor,"xa=f"=>\$xa,"xb=f"=>\$xb,"ya=f"=>\$ya,"yb=f"=>\$yb,"za=f"=>\$za,"zb=f"=>\$zb,\
            ...
            "edgeHeight=f"=>\$edgeHeight,"edgeWidth=f"=>\$edgeWidth,"numGhost=i"=>\$numGhost,"ds0=f"=>\$ds0 );
# 
...
Box
  ...
  lines
    ...
    $nx $ny $nz
  boundary conditions
    if( $per eq 1 ){ $cmd=" 1 2 -1 -1 -1 -1"; }else{ $cmd="1 2 3 4 5 6"; }
    $cmd 
  share
      0 0 0 0 5 6 
  mappingName
...
#
  ...
  hyperbolic
    forward
    $nDist=($nr-4)*$ds;
    # fix needed for IBeam -factor=8: 
    if( $shape eq "IBeam" ){ $nr = intmg( 5 + $order/2 );  $nDist=($nr-2)*$ds; $arcLength=$arcLength*1.25; }
    #
...
Box
  ...
  lines
    ...
    $nx $ny $nz
  boundary conditions
    if( $per eq 0 ){ $cmd=" 0 0 0 0 5 6"; }else{ $cmd="0 0 0 0 -1 -1"; }
    $cmd    
  share 
    0 0 0 0 5 6
  mappingName
...
convertToNurbs("outerShape0","outerShape2d",$angle);
$commands
convertToNurbs("innerShape0","innerShape2d",$angle);
$commands
#
# Extrude into 3D
# 
 sweep
    ...
    lines
      $nTheta $nr $nz
    boundary conditions
      if( $per eq 0 ){ $bcCommand =" -1 -1 7 0 5 6"; }else{ $bcCommand = " -1 -1 7 0 -1 -1"; }
      $bcCommand
    share 
       0 0 100 0 5 6 
  mappingName
