#================================================================================================
#
#  cgmx example:  SCATTERING FROM SLABS
echo to terminal 1
#
# Usage:
#   
#  cgmx [-noplot] slabs -g=<name> -tf=<tFinal> -tp=<tPlot> -kx=<num> -ky=<num> -kz=<num> ...
...
#-if( $numSlabs eq 0 ){ $cmd="GDM params $a0 $a1 $b0 $b1 innerDomain (a0,a1,b0,b1,domain-name)\n"; }\
#-else{\
#-  $cmd="#"; \
#-  for( $i=0; $i<$numSlabs; $i++ ){ $cmd .= "\n GDM params $a0 $a1 $b0 $b1 blockDomain$i (a0,a1,b0,b1,domain-name)"; }\
#-}
#-$cmd 
#
$kxa= abs($kx);
...
#- @muv = ( $mu1, $mu2, $mu3, $mu4 );
#- if( $numSlabs eq 0 ){ $cmd="coefficients $eps1 $mu1 innerDomain   (eps,mu,grid/domain-name)\n"; }\
#- else{\
#-   $cmd="#"; \
