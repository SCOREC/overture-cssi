#
# cgmp:   Incompressible flow past an elastic disk
# 
# Usage:
#    cgmp [-noplot] insElasticDisk -g=<name> -method=[ins|cns] -nu=<> -mu=<> -kappa=<num> -tf=<tFinal> -tp=<tPlot> ...
...
# 
$grid="deformingChannelGrid4.order2"; $domain1="outerDomain"; $domain2="innerDomain";
...
$nu=.1; $rhoSolid=1.; $prandtl=.72; $cnsVariation="jameson"; $ktcFluid=-1.; $u0=1.; $p0=1.;
$p0=1.; 
$cnsEOS="ideal"; 
...
$ra=.01; $rb=2.1; # ramp time interval 
...
GetOptions( "g=s"=>\$grid,"tf=f"=>\$tFinal,"nu=f"=>\$nu,"muFluid=f"=>\$muFluid,"kappa=f"=>\$kappa, "bg=s"=>\$backGround,\
 ...
 "show=s"=>\$show,"method=s"=>\$method,"ts=s"=>\$ts,"tsSM=s"=>\$tsSM,"noplot=s"=>\$noplot,"ktcFluid=f"=>\$ktcFluid,\
  ...
  "cfl=f"=>\$cfl,"rhoSolid=f"=>\$rhoSolid,"cnsVariation=s"=>\$cnsVariation,"diss=f"=>\$diss,"fic=s"=>\$fic,"go=s"=>\$go,\
   ...
   "freqFullUpdate=i"=>\$freqFullUpdate,"smoothInterface=i"=>\$smoothInterface,"rampInflow=i"=>\$rampInflow,\
...
#
    # **** ramp the pressure on the left ****
    $rampBC="bcNumber1=outflow, pressure(1.*p+0.*p.n=$p0), userDefinedBoundaryData\n" . \
    " time function option\n" . \
    "   ramp function\n" .\
    "   ramp end values: 0,1 (start,end)\n" .\
    "   ramp times: $ra,$rb (start,end)\n" .\
    "   ramp order: 3\n" .\
    " exit \n" .\
    "done";
#
...
$bc = "all=noSlipWall uniform(u=.0,T=$T0)\n bcNumber3=slipWall\n bcNumber4=slipWall\n bcNumber1=inflowWithPressureAndTangentialVelocityGiven, uniform(p=$p0,v=0.,T=0.)\n bcNumber2=outflow, pressure(1.*p+.0*p.n=0.)\n bcNumber100=tractionInterface";
if( $option eq "halfDisk" ){ $bc = "all=noSlipWall uniform(u=.0,T=$T0)\n bcNumber3=slipWall\n bcNumber4=slipWall\n bcNumber1=inflowWithPressureAndTangentialVelocityGiven, uniform(p=$p0,v=0.,T=0.)\n bcNumber2=outflow, pressure(1.*p+.0*p.n=0.)\n bcNumber100=tractionInterface"; }
#
if( $rampInflow eq 1 ){ $p0=0.; $u0=0.; $bc = "all=noSlipWall uniform(u=.0,T=$T0)\n bcNumber3=noSlipWall\n bcNumber4=noSlipWall\n bcNumber1=inflowWithVelocityGiven, ramp(ta=$ra,tb=$rb,ua=0,ub=.1)\n bcNumber2=outflow, pressure(1.*p+$apn*p.n=0.)\n bcNumber100=tractionInterface"; }
if( $rampInflow eq 1 ){ $p0=0.; $u0=0.; $bc = "all=noSlipWall uniform(u=.0,T=$T0)\n bcNumber3=slipWall\n bcNumber4=slipWall\n $rampBC\n bcNumber2=outflow, pressure(1.*p+$apn*p.n=0.)\n bcNumber100=tractionInterface"; }
#
$ic="uniform flow\n" . "p=$p0, u=$u0, T=$T0";
#
