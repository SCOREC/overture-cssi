// This file automatically generated from outputProbes.bC with bpp.
...
#define bb(side,axis) boundingBox[(side)+2*(axis)]    
  ...
  // These probes are defined from an average or integral over a sub-domain.
    ...
    // -- these next values should probably be stored with the probe: 
        ...
        // Look for which probes are in this probe.file
                int savePosition=false;
                int numberOfEntries=0;   // counts field components that we output 
      	for( int j=0; j<numberOfProbeRegions; j++ )
      	{
        	  if( probeRegionFileNumber[j]==pf )
        	  {
          	    ...
          	    if( probe.probeType==ProbeInfo::probeBoundarySurface ||
            		...
            		numberOfProbeRegionValues(pr)=numberOfComponents;	
                        ...
                        numberOfEntries+=numberOfProbeRegionValues(pr);
	...
	// *wdh* Jan 14, 2020 int numHeader=4+numberOfEntries;        // number of header comments 
      	int numHeader=5;                        // number of header comments 
...
// 	}
        ...
        int numberOfIntegrationRegions=0; // counts number of integral probes
        for( int pr=0; pr<numberOfProbeRegions; pr++ )
        {
            const int i = probeRegionIndex[pr];
            assert( probeList[i]!=NULL );
      ...
      // --- loop over different probe regions ---
        ...
        // assert( file!=NULL );
      	...
      	if( measureType=="integral" )
      	{
            ...
            // ------ coordinate plane probe -----
              ...
              // store integrals in val[c] : 
                            real *val = new real[numberOfComponents+1];  // add one for surface area, used in parallel sum below
            	      for( int c=0; c<numberOfComponents; c++ )
            		val[c]=0.;
              ...
              // tangential index directions: 
                            ...
                            const real deltaArea  = numberOfDimensions==2 ? dvx[axisp1] : dvx[axisp1]*dvx[axisp2];  // line or area element

            	      real surfaceArea=0.;   // compute the surface area here  
	      ...
	      // surfaceArea *= deltaArea;
                            ...
                            val[numberOfComponents]=surfaceArea;      // do this so we sum in parallel below
            	      
              // sum across processors 
            	      real *sum = new real[numberOfComponents+1];
            	      ParallelUtility::getSums( val,sum,numberOfComponents+1 );
            	      
                            surfaceArea = sum[numberOfComponents];
            	      for( int c=0; c<numberOfComponents; c++ )
            		value(pr,c)=sum[c];

            	      if( quantityMeasure=="average" )
            	      {
                // normalize integrals by the surface area.
            		for( int c=0; c<numberOfComponents; c++ )
              		  value(pr,c) /= surfaceArea;
            	      }
                            ...
                            {
                                if( quantityMeasure=="average" )
                                    printF("outputProbes: surfaceArea=%12.4e, integrals/surfaceArea=",surfaceArea);
                                else
