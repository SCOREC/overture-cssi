// This file automatically generated from DeformingBodyMotion.bC with bpp.
// ---------------------------------------------------------------------------------------
        ...
        // macro: 
                    ...
                    if( startCurveType==nurbsStartCurve )
                    {
                        if( false )
                          ...
                          int option=0, degree=3;
                          const int boundaryParameterization = deformingBodyDataBase.get<int>("boundaryParameterization");
                          ((NurbsMapping&)startCurve).interpolate(xPast,option,Overture::nullRealDistributedArray(),degree,
                                                                                                          (NurbsMapping::ParameterizationTypeEnum)boundaryParameterization,numGhost);
                        #endif
                      }
                      else if( startCurveType==splineStartCurve )
                      {
                          ...
                          RealArray x(n0),y(n0);
                          Range all;
                          x=xPast(all,0); y=xPast(all,1);
                          spline.setPoints( x,y );  
             // can we do this? spline.setPoints( xPast(all,0),xPast(all,1) );  
                      }
                      else
                      {
                          OV_ABORT("Unknown start curve type");
      ...
      // macro: 
                ...
                if( startCurveType==nurbsStartCurve )
                {
                    if( false )
                      ...
                      int option=0, degree=3;
                      const int boundaryParameterization = deformingBodyDataBase.get<int>("boundaryParameterization");
                      ((NurbsMapping&)startCurve).interpolate(x3,option,Overture::nullRealDistributedArray(),degree,
                                                                                                      (NurbsMapping::ParameterizationTypeEnum)boundaryParameterization,numGhost);
                    #endif
                  }
                  else if( startCurveType==splineStartCurve )
                  {
                      ...
                      RealArray x(n0),y(n0);
                      Range all;
                      x=x3(all,0); y=x3(all,1);
                      spline.setPoints( x,y );  
           // can we do this? spline.setPoints( x3(all,0),x3(all,1) );  
                  }
                  else
                  {
                      OV_ABORT("Unknown start curve type");
      ...
      // macro: 
                ...
                if( startCurveType==nurbsStartCurve )
                {
                    if( false )
                      ...
                      int option=0, degree=3;
                      const int boundaryParameterization = deformingBodyDataBase.get<int>("boundaryParameterization");
                      ((NurbsMapping&)startCurve).interpolate(xbb,option,Overture::nullRealDistributedArray(),degree,
                                                                                                      (NurbsMapping::ParameterizationTypeEnum)boundaryParameterization,numGhost);
                    #endif
                  }
                  else if( startCurveType==splineStartCurve )
                  {
                      ...
                      RealArray x(n0),y(n0);
                      Range all;
                      x=xbb(all,0); y=xbb(all,1);
                      spline.setPoints( x,y );  
           // can we do this? spline.setPoints( xbb(all,0),xbb(all,1) );  
                  }
                  else
                  {
                      OV_ABORT("Unknown start curve type");
        ...
        // macro: 
                    ...
                    if( startCurveType==nurbsStartCurve )
                    {
                        if( false )
                          ...
                          int option=0, degree=3;
                          const int boundaryParameterization = deformingBodyDataBase.get<int>("boundaryParameterization");
                          ((NurbsMapping&)startCurve).interpolate(x2,option,Overture::nullRealDistributedArray(),degree,
                                                                                                          (NurbsMapping::ParameterizationTypeEnum)boundaryParameterization,numGhost);
                        #endif
                      }
                      else if( startCurveType==splineStartCurve )
                      {
                          ...
                          RealArray x(n0),y(n0);
                          Range all;
                          x=x2(all,0); y=x2(all,1);
                          spline.setPoints( x,y );  
             // can we do this? spline.setPoints( x2(all,0),x2(all,1) );  
                      }
                      else
                      {
                          OV_ABORT("Unknown start curve type");
        ...
        // macro: 
                    ...
                    if( startCurveType==nurbsStartCurve )
                    {
                        if( false )
                          ...
                          int option=0, degree=3;
                          const int boundaryParameterization = deformingBodyDataBase.get<int>("boundaryParameterization");
                          ((NurbsMapping&)startCurve).interpolate(x1,option,Overture::nullRealDistributedArray(),degree,
                                                                                                          (NurbsMapping::ParameterizationTypeEnum)boundaryParameterization,numGhost);
                        #endif
                      }
                      else if( startCurveType==splineStartCurve )
                      {
                          ...
                          RealArray x(n0),y(n0);
                          Range all;
                          x=x1(all,0); y=x1(all,1);
                          spline.setPoints( x,y );  
             // can we do this? spline.setPoints( x1(all,0),x1(all,1) );  
                      }
                      else
                      {
                          OV_ABORT("Unknown start curve type");
