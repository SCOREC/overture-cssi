      subroutine cmpfluxMG1d (m,wl,wr,fl,fr,speed,method,linsc)
c
...
c
      ...
      dimension rm1(2,2),vm1(2,2),pm1(2,2),sm1(2,2)
...
c
c assign left state
      alpha(1,1)=wl(1)             ! vol, solid
      alpha(1,2)=1.d0-wl(1)        ! vol, gas
      r(1,1)=wl(2)                 ! density, solid
      r(1,2)=wl(5)                 ! density, gas
      v(1,1)=wl(3)                 ! velocity, solid
      v(1,2)=wl(6)                 ! velocity, gas
      p(1,1)=wl(4)                 ! pressure, solid
      p(1,2)=wl(7)                 ! pressure, gas
c
c assign right state
      alpha(2,1)=wr(1)             ! vol, solid
      alpha(2,2)=1.d0-wr(1)        ! vol, gas
      r(2,1)=wr(2)                 ! density, solid
      r(2,2)=wr(5)                 ! density, gas
      v(2,1)=wr(3)                 ! velocity, solid
      v(2,2)=wr(6)                 ! velocity, gas
      p(2,1)=wr(4)                 ! pressure, solid
      p(2,2)=wr(7)                 ! pressure, gas
c
      ...
      do j=1,2
        ...
        do i=1,2
          if (r(i,j).le.0.d0) then
            write(6,*)'Error (gdflux) : r(i,j).le.0, i,j=',i,j
            stop
          end if
          Volij=1.d0/r(i,j)
          cm2=c2mg(kj,Volij,p(i,j))
          if (cm2.lt.0.d0) then
            write(6,*)'Error (gdflux) : cm2.lt.0, i,j=',i,j
            stop
          end if
          c(i,j)=dsqrt(cm2)
...
c
c..compute fluxes
        call cmpflxmg (m,rm0,rm,vm,pm,sm,wl,wr,fl,fr,method)
c
...
c     
c..   compute fluxes
         call cmpflxhc (m,rm0,rm,vm,pm,sm,wl,wr,fl,fr)
c
      else
        write(6,*)'Error (gdflux) : value of method not supported'
        stop
      end if
c
...
c+++++++++++++++
c
      subroutine cmpflxhc (m,rm0,rm,vm,pm,sm,wl,wr,fl,fr)
c
...
c solid phase flux components
        call getfxhc (1,1,rm0,rm,vm,pm,sm,fl(2))
c
...
c gas phase flux components
        call getfxhc (1,2,rm0,rm,vm,pm,sm,fl(5))
c
c add on solid contact source contributions
        fr(5)=fl(5)
        fr(6)=fl(6)-source
...
c
c solid phase flux components
        call getfxhc (2,1,rm0,rm,vm,pm,sm,fr(2))
c
c add on solid contact source contributions
        fl(2)=fr(2)
        fl(3)=fr(3)-source
...
c
c gas phase flux components
        call getfxhc (2,2,rm0,rm,vm,pm,sm,fr(5))
c
c add on solid contact source contributions
        fl(5)=fr(5)
        fl(6)=fr(6)+source
...
c+++++++++++++++
c
      subroutine getfxhc (i,j,rm0,rm,vm,pm,sm,fx)
c
c compute solid or gas flux for i=side and j=phase
c
      implicit double precision (a-h,o-z)
      dimension rm(2,2),vm(2,2),pm(2,2),sm(2,2),fx(3)
      dimension uk(3),um(3),u0(3)
      common / prmdatmg / alpha(2,2),r(2,2),v(2,2),p(2,2),c(2,2),
     *                    gamk(2,2),p0k(2,2)
      common / gasdatmg / gam(2),gm1(2),p0(2)
      common / matdatmg / kmat(2)
...
c compute flux
      fx(1)=uk(2)
      fx(2)=fx(1)*v(i,j)+alpha(i,j)*p(i,j)
      fx(3)=(uk(3)+alpha(i,j)*p(i,j))*v(i,j)
c
...
c middle state 0 for the gas phase
          ...
          en0=rm0*(A0*pm(i,j)+B0+.5d0*vm(i,j)**2)
          u0(3)=alpha(i,j)*en0
          do n=1,3
...
c+++++++++++++++
c
      subroutine cmpflxmg (m,rm0,rm,vm,pm,sm,wl,wr,fl,fr,method)
c
c compute fluxes based on coupled middle states
c
      implicit real*8 (a-h,o-z)
...
c
c solid phase flux components
        call getfxmg (1,1,rm0,rm,vm,pm,sm,wl(2),fl(2),method)
c
c add on solid contact source contributions
        fr(2)=fl(2)
        fr(3)=fl(3)+source
...
c
c gas phase flux components
        call getfxmg (1,2,rm0,rm,vm,pm,sm,wl(5),fl(5),method)
c
c add on solid contact source contributions
        fr(5)=fl(5)
        fr(6)=fl(6)-source
...
c
c solid phase flux components
        call getfxmg (2,1,rm0,rm,vm,pm,sm,wr(2),fr(2),method)
c
c add on solid contact source contributions
        fl(2)=fr(2)
        fl(3)=fr(3)-source
...
c
c gas phase flux components
        call getfxmg (2,2,rm0,rm,vm,pm,sm,wr(5),fr(5),method)
c
c add on solid contact source contributions
        fl(5)=fr(5)
        fl(6)=fr(6)+source
...
c+++++++++++++++
c
      subroutine getfxmg (i,j,rm0,rm,vm,pm,sm,wstar,fx,method)
c
c compute solid or gas flux for i=side and j=phase
c
      implicit double precision (a-h,o-z)
      dimension rm(2,2),vm(2,2),pm(2,2),sm(2,2),wstar(3),fx(3)
      common / prmdatmg / alpha(2,2),r(2,2),v(2,2),p(2,2),c(2,2),
     *                    gamk(2,2),p0k(2,2)
      common / gasdatmg / gam(2),gm1(2),p0(2)
...
c middle left (i=1) or middle right (i=2)
          ...
          wstar(2)=vm(i,j)
          wstar(3)=pm(i,j)
          if (j.eq.2.and.isign*vm(i,2).lt.0.d0) then
c use gas density between solid and gas contacts
            wstar(1)=rm0
          end if
        end if
      else
...
c sonic left (i=1) or right (i=2)
            if (method.eq.0) then
              call sonicmg (i,j,rm,vm,pm,sm,wstar)
            elseif (method.eq.1) then
...
c
c add kinetic energy
      energy=wstar(1)*(energy+.5d0*wstar(2)**2)
c
c compute flux
      fx(1)=alpha(i,j)*wstar(1)*wstar(2)
      fx(2)=fx(1)*wstar(2)+alpha(i,j)*wstar(3)
      fx(3)=alpha(i,j)*(energy+wstar(3))*wstar(2)
c
      return
      end
c
c+++++++++++++++
c
      subroutine sonicmg (i,j,rm,vm,pm,sm,wstar)
c
c compute solid or gas flux for i=side and j=phase
c
      implicit double precision (a-h,o-z)
      dimension rm(2,2),vm(2,2),pm(2,2),sm(2,2),wstar(3)
      common / prmdatmg / alpha(2,2),r(2,2),v(2,2),p(2,2),c(2,2),
     *                    gamk(2,2),p0k(2,2)
      common / matdatmg / kmat(2)
