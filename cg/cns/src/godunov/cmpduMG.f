...
c
c Multi-phase flow, BN equations, with advected scalars, MG equations of state
c
c NOTE: this file originated from cmpduv.f, but then added the capability to handle
c       general MG equations of state.
c
c NOTE: the pointer to the data base for accessing parameter values has been
c       added to the variables.  Presently, this not used but it available.
c   *** cmpduv.f does not include this pointer so this code is not compatible ***
c
     ...
     *          rwk(nrwk),iwk(niwk)
      ...
      integer ok,getInt,getReal  ! data base utilities
c
      dimension ic(7)
...
c
c      kmat(1)=4   ! stiffened gas (solid phase)
c      kmat(2)=5   ! virial gas    (gas phase)
c
...
c
      if (m.lt.0) then
        if (imgeos.eq.0) then
          ...
          call chkpMG (m,nd1a,nd1b,md1a,md1b,nd2a,nd2b,md2a,md2b,u,rwk)
        end if
      end if
c
...
c      pdb=rparam(16)
c
      if (nstep.lt.0.and.igrid.eq.0) then
c
c get EOS flag, imgeos=0 => stiffend solid and virial gas (original code), imgeos.ne.0 => MG code
c        imgeos=0
c        ok=getInt(pdb,'PdeParameters/iMGEOS',imgeos)
c        if (imgeos.eq.0) then
c          write(6,*)'EOS model: stiffend solid and virial gas'
c        else
c          kmat1=4
c          ok=getInt(pdb,'PdeParameters/kmatSolid',kmat1)
c          write(6,110)
c  110     format('=> Material for phase 1 (solid):')
c          call mgeosi (1,kmat1)
c          kmat2=5
c          ok=getInt(pdb,'PdeParameters/kmatGas',kmat2)
c          write(6,120)
c  120     format('=> Material for phase 2 (gas):')
c          call mgeosi (2,kmat2)
c          methodMG=0
c          linsc=0
c          ok=getInt(pdb,'PdeParameters/methodMG',methodMG)
c          ok=getInt(pdb,'PdeParameters/linsc',linsc)
c          if (methodMG.eq.0) then
c            write(6,*)'Exact Riemann solver, linsc=',linsc
c          elseif (methodMG.eq.1) then
c            write(6,*)'Locally stiffened Riemann solver, linsc=',linsc
c          elseif (methodMG.eq.2) then
c            write(6,*)'HLLC Riemann solver, linsc=',linsc
c          else
c            write(6,*)'Error (cmpdu) : methodMG not supported'
c            stop
c          end if
c        end if
        if (imgeos.eq.0) then
          write(6,*)'Stiffened solid, virial gas (original code)'
        else
          write(6,*)'Mie-Gruneisen EOS (new code)'
          if (methodMG.eq.0) then
            if (linsc.eq.0) then
              write(6,*)'Exact Riemann solver, linsc OFF'
            else
              write(6,*)'Exact Riemann solver, linsc ON'
            end if
          elseif (methodMG.eq.1) then
            if (linsc.eq.0) then
              write(6,*)'Locally stiffened solver, linsc OFF'
            else
              write(6,*)'Locally stiffened solver, linsc ON'
            end if
          elseif (methodMG.eq.2) then
            if (linsc.eq.0) then
              write(6,*)'HLLC solver, linsc OFF'
            else
              write(6,*)'HLLC solver, linsc ON'
            end if
          else
            write(6,*)'Error (cmpdu) : methodMG not supported'
            stop
          end if
        end if
        if (iorder.eq.1) then
          write(6,*)'Slope correction is OFF'
        else
          write(6,*)'Slope correction is ON'
        end if
        if (itz.eq.0) then
          write(6,*)'TZ is OFF'
          if (isrc.eq.0) then
            write(6,*)'Source terms are OFF'
          else
            write(6,*)'Source terms are ON'
          end if
        else
          write(6,*)'TZ is ON'
        end if
c        read(5,*)ians
c        ixy=1
c        if (dabs(u(4,5,1)-u(4,4,1)).gt.1.d-8) ixy=2
      end if
c
c**** temp ****
c      itz=0
...
c
     ...
     *               u,mask,rwk,ier,1)
          ...
          call chksMG (m,nd1a,nd1b,md1a,md1b,nd2a,nd2b,md2a,md2b,
     *                 u,mask,rwk,ier,1)
        end if
        if (ier.ne.0) then
      ...
      do j1=md1a,md1b
        if (mask(j1,j2).ne.0) then
          ...
          if (rs.le.rhoMin) then
            iflag=1
            write(6,400)j1,j2
  400       format(' ** Error (cmpdu) : very small solid density, ',
     *             'j1,j2 =',2(1x,i4))
          ...
          if (rg.le.rhoMin) then
            iflag=1
            write(6,410)j1,j2
  410       format(' ** Error (cmpdu) : very small gas density, ',
     *             'j1,j2 =',2(1x,i4))
...
c truncation error estimate returned by cmpsource)
      if (isrc.ne.0.and.itz.eq.0) then
        dr2=.5d0*dr
        do k=1,ntau
          tau(k)=0.d0
        end do
        if (imgeos.eq.0) then
          call cmpsource (m,nd1a,nd1b,md1a,md1b,nd2a,nd2b,md2a,md2b,
     *                    dr2,up,mask,tau,maxstep1,niwk,iwk,nrwk,rwk)
        else
          call cmpsourceMG (m,nd1a,nd1b,md1a,md1b,nd2a,nd2b,md2a,md2b,
     *                      dr2,up,mask,tau,maxstep1,niwk,iwk,nrwk,rwk)
        end if
      end if
c
...
c compute second source contribution
      if (isrc.ne.0.and.itz.eq.0) then
        if (imgeos.eq.0) then
          call cmpsource (m,nd1a,nd1b,n1a,n1b,nd2a,nd2b,n2a,n2b,
     *                    dr2,up,mask,tau,maxstep2,niwk,iwk,nrwk,rwk)
        else
          call cmpsourceMG (m,nd1a,nd1b,n1a,n1b,nd2a,nd2b,n2a,n2b,
     *                      dr2,up,mask,tau,maxstep2,niwk,iwk,nrwk,rwk)
        end if
c       write(6,*)'** cmpSource : maximum time steps = ',
c    *            max(maxstep1,maxstep2)
c        call setdat (m,nd1a,nd1b,md1a,md1b,n1a,n1b,nd2a,nd2b,
c     *               md2a,md2b,n2a,n2b,tau,mdat,dat)
      end if
c
      if (m.lt.0) then
        if (imgeos.eq.0) then
...
c
      if (m.gt.0) then
        write(6,*)'cmplowfix0 not fixed yet'
        stop
      end if
c
      do j2=md2a,md2b
...
c
      if (m.gt.0) then
        write(6,*)'cmplowfix not fixed yet'
        stop
      end if
c
      iflag=0
...
c***************
c
      subroutine chkpMG (m,nd1a,nd1b,md1a,md1b,nd2a,nd2b,md2a,md2b,u,w)
c
      implicit real*8 (a-h,o-z)
      dimension u(nd1a:nd1b,nd2a:nd2b,m),w(m,md1a:md1b)
c
      do j2=md2a,md2b
        call cmppvsMG (m,nd1a,nd1b,md1a,md1b,nd2a,nd2b,j2,u,w)
        do j1=md1a,md1b
          if (w(4,j1).le.0.d0.or.w(8,j1).le.0.d0) then
            write(6,100)nd1a,nd1b,n1a,n1b,nd2a,nd2b,n2a,n2b,
     *                  j1,j2,(i,w(i,j1),i=1,m)
...
c
      if (ier.eq.0) return
c
      write(55,100)loc,m,nd1a,nd1b,md1a,md1b,nd2a,nd2b,md2a,md2b
...
c***************
c
      subroutine chksMG (m,nd1a,nd1b,md1a,md1b,nd2a,nd2b,md2a,md2b,
     *                   u,mask,w,ier,loc)
c
      implicit real*8 (a-h,o-z)
      dimension u(nd1a:nd1b,nd2a:nd2b,m),mask(nd1a:nd1b,nd2a:nd2b),
...
c
      ...
      do j2=md2a,md2b
        ...
        do j1=md1a,md1b
          if (mask(j1,j2).ne.0) then
            Vol=1.d0/w(1,j1)
            cb2=c2mg(kmat(1),Vol,w(4,j1))
            if (cb2.lt.c2min) then
              ier=1
              write(6,*)'j1,j2,cb2=',j1,j2,cb2
            end if
            Vol=1.d0/w(5,j1)
            cb=c2mg(kmat(2),Vol,w(8,j1))
            if (c2.lt.c2min) then
              ier=2
              write(6,*)'j1,j2,c2=',j1,j2,c2
            end if
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c
c               Source step (Original Version)
c
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c
c               Source step (MG Version)
c
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c
      subroutine cmpsourceMG (m,nd1a,nd1b,n1a,n1b,nd2a,nd2b,n2a,n2b,
     *                        dt,u,mask,tau,maxstep1,niwk,iwk,nrwk,rwk)
c
c Solid EOS includes a compaction potential.
c
c source contribution for drag, compaction, heat transfer and chemical
c reaction.  The drag coefficient is delta, the compaction viscosity is
c 1/rmuc and the heat transfer coefficient is htrans.
c
c Integrate the source term from t=0 to t=dt.
c
      implicit real*8 (a-h,o-z)
      dimension u(nd1a:nd1b,nd2a:nd2b,m),mask(nd1a:nd1b,nd2a:nd2b),
     *          tau(nd1a:nd1b,nd2a:nd2b),iwk(niwk),rwk(nrwk)
...
c
c first quarter step
      dt2=.5d0*dt
      if (dabs(htrans).lt.tol) then
c drag only (can use original, non-MG code since only kinematics)
        call cmpsrc1 (m,nd1a,nd1b,n1a,n1b,nd2a,nd2b,n2a,n2b,dt2,
     *                u,mask)
      else
        if (dabs(delta).gt.tol) then
c drag and heat transfer
          call cmpsrc1MG (m,nd1a,nd1b,n1a,n1b,nd2a,nd2b,n2a,n2b,dt2,
     *                    u,rwk,mask)
        else
          write(6,*)'Error (cmpsourceMG) : if heat transfer is on, ',
     *              'then drag must also be on'
          stop
        end if
      end if
c
...
c half step: compaction, heat transfer and chemical reaction
      ...
      maxstep1=0
      do j2=n2a,n2b
        call cmpsrc2MG (m,nd1a,nd1b,n1a,n1b,nd2a,nd2b,j2,dt,u,
     *                mask(nd1a,j2),tau(nd1a,j2),nd,iwk(ljk),rwk(ltk),
     *                rwk(lhk),rwk(luk),rwk(lqk),rwk(lwk),rwk(lupk),
     *                rwk(luk1),rwk(ltest),iwk(lnstep),mpk,maxstep,
     *                ier)
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++
c
      subroutine cmpsrc1MG (m,nd1a,nd1b,n1a,n1b,nd2a,nd2b,n2a,n2b,dt,
     *                      u,w,mask)
c
c source step: drag and heat transfer only (note: both delta and htrans must
c              be away from zero to avoid interminant forms in the exact
c              solution)
c
      implicit real*8 (a-h,o-z)
      dimension u(nd1a:nd1b,nd2a:nd2b,m),w(m,nd1a:nd1b),
...
c
      do j2=n2a,n2b
c
        call cmppvsMG (m,nd1a,nd1b,n1a,n1b,nd2a,nd2b,j2,u,w)
c
        do j1=n1a,n1b
          if (mask(j1,j2).ne.0.and.u(j1,j2,9).gt.abmin1) then
c
...
c current velocities
            ...
            dv0=vg0-vs0
c
c current temperatures
            Vs0=1.d0/w(1,j1)
            ts0=TmgV(1,kmat(1),Vs0,w(4,j1),jer)
            if (jer.ne.0) then
              write(6,*)'Error (cmpsrc1MG) : error return from TmgV(s)'
              stop
            end if
            Vg0=1.d0/w(5,j1)
            tg0=TmgV(2,kmat(2),Vg0,w(8,j1),jer)
            if (jer.ne.0) then
              write(6,*)'Error (cmpsrc1MG) : error return from TmgV(g)'
              stop
            end if
            dtemp0=tg0-ts0
c
c current specific heats
            Cvs=Cvmg(kmat(1),Vs0,ts0)
            Cvg=Cvmg(kmat(2),Vg0,tg0)
c
c reciprocal time scales
            alamv=delta*(1.d0/fs+1.d0/fg)
            alam2=2.d0*alamv
            alamh=htrans*(1.d0/(Cvs*fs)+1.d0/(Cvg*fg))
            if (dabs(alam2-alamh).le.1.d-8) then
              write(6,*)'Error (cmpsrc1MG) : 2*alamv.eq.alamh ???'
c See: MG-SourceTermsNotes2D.pdf for a possible resolution of this
c      special case.  For now, there is no resolution implemented
              stop
            end if
c
c exponential terms
            expn1=(1.d0-dexp(-alamv*dt))/alamv
            expn2=(1.d0-dexp(-alam2*dt))/alam2
            expn3=(1.d0-dexp(-alamh*dt))/alamh
c
c some constants
            cd=delta*(du0**2+dv0**2)
            ce=delta*cd/(fs*alamv)
            ch=htrans*cd/(fg*Cvg*(alam2-alamh))
            fact1=delta*(du0*us0+dv0*uv0)+ce                !  A in the notes
            fact2=ce+ch                                     ! -B in the notes
            fact3=htrans*dtemp0+ch                          !  C in the notes
c
c momentum and energy increments
            dm1=delta*du0*expn1
            dm2=delta*dv0*expn1
            de=fact1*expn1-fact2*expn2+fact3*expn3
c
c update conserved variables
            u(j1,j2,2)=u(j1,j2,2)+dm1
            u(j1,j2,3)=u(j1,j2,3)+dm2
            u(j1,j2,4)=u(j1,j2,4)+de
            u(j1,j2,6)=u(j1,j2,6)-dm1
            u(j1,j2,7)=u(j1,j2,7)-dm2
            u(j1,j2,8)=u(j1,j2,8)-de
c
          end if
        end do
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++
c
      subroutine cmpsrc2MG (m,nd1a,nd1b,n1a,n1b,nd2a,nd2b,j2,dt,u,
     *                      mask,tau,nd,jk,tk,hk,uk,qk,wk,upk,uk1,
     *                      test,nstep,mpk,maxstep,ier)
c
c source step: compaction, heat transfer, chemical reaction
...
c time steps for t=0 to t=dt
      do it=1,itmax
c
        call cmprateMG (m,nd,n,uk,wk,upk(1,1,1),mpk)
c
        do k=1,n
          do i=1,4                              ! update conserved solid and gas quantities
            upk(i,k,1)=hk(k)*upk(i,k,1)
...
c
        call cmprateMG (m,nd,n,uk1,wk,upk(1,1,2),mpk)
c
        do k=1,n
          do i=1,4                              ! update conserved solid and gas quantities
            upk(i,k,2)=hk(k)*upk(i,k,2)
...
c
        call cmprateMG (m,nd,n,uk1,wk,upk(1,1,3),mpk)
c
        do k=1,n
          j1=jk(k)
          test(k)=0.d0
          Vol=1.d0/wk(5,k)
          c2=c2mg(kmat(2),Vol,wk(8,k))
          c=dsqrt(max(c2,c2min))
          Vol=1.d0/wk(1,k)
          cb2=c2mg(kmat(1),Vol,wk(4,k))
          cb=dsqrt(max(cb2,c2min))
          scale(1)=qk(k,1)
          scale(2)=qk(k,1)*max(dabs(wk(2,k))+cb,dabs(wk(6,k))+c)
          scale(3)=qk(k,1)*max(dabs(wk(3,k))+cb,dabs(wk(7,k))+c)
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++
c
      subroutine cmprateMG (m,nd,n,uk,wk,upk,mpk)
c
c MG version: compaction and chemical reaction only
c
      implicit real*8 (a-h,o-z)
      dimension uk(nd,m),wk(m,nd),upk(mpk,nd)
      common / srcprm / delta,rmuc,htrans,cratio,abmin,abmax,isrc
...
c
c get primitive state
      call cmppvsMG (m,1,nd,1,n,1,1,1,uk,wk)
c
c get rate
      do k=1,n
c
...
c
      dimension ic(7),icw(3),wsavel(9),wsaver(9),flchk(9),frchk(9)
c      dimension w1c(9,-2:13,-2:13,5)
c
      dimension fl(mlocal),fr(mlocal),htz(mlocal)
      common / axidat / iaxi,j1axi(2),j2axi(2)
      common / tzflow / eptz,itz
c
      common / gasdat / gam(2),gm1(2),gp1(2),em(2),ep(2),ps0,bgas
...
c     data ic / 9,1,2,4,5,6,8 /
      ...
      data icw / 2,3,9 /
c
c flag for new MG EOS code (0=>orig, 1=MG EOS)
c      data imgeos / 1 /
c
c temp: set method and linsc for mg flux
      chktol=1.d-6
c      methodMG=2
c      linsc=0
c
      if (m.gt.mlocal) then
        write(6,*)'Error (cmphydro) : m.gt.mlocal'
        stop
      end if
c
c..ratios
...
c..begin hydro step.  Get primitive states along lines j2=n2a-2,n2a-1,n2a
     ...
     *                 w(1,md1a,k2))
        ...
        do k2=1,3
          j2=n2a+k2-3
          call cmppvsMG (m,nd1a,nd1b,md1a,md1b,nd2a,nd2b,j2,up,
     *                   w(1,md1a,k2))
        end do
      end if
c
...
c..slope correction (bottom row of cells)
      if (imgeos.eq.0) then
        call cmpslope (m,nd1a,nd1b,md1a,md1b,n1a,n1b,nd2a,nd2b,
     *                 j2,r,dr,ds,xy,a0(1,md1a,1),a1(1,1,md1a,1),
     *                 aj(md1a,1),mask(nd1a,j2),w,w1(1,md1a,1,1),
     *                 iorder)
      else
        call cmpslopeMG (m,nd1a,nd1b,md1a,md1b,n1a,n1b,nd2a,nd2b,
     *                   j2,r,dr,ds,xy,a0(1,md1a,1),a1(1,1,md1a,1),
     *                   aj(md1a,1),mask(nd1a,j2),w,w1(1,md1a,1,1),
     *                   iorder)
      end if
...
c..get primitive states along line j2+1
     ...
     *                 w(1,md1a,3))
          ...
          call cmppvsMG (m,nd1a,nd1b,md1a,md1b,nd2a,nd2b,j2+1,up,
     *                   w(1,md1a,3))
        end if
c
c..set grid metrics and velocity (if necessary)
...
c..slope correction (top row of cells)
        if (imgeos.eq.0) then
          call cmpslope (m,nd1a,nd1b,md1a,md1b,n1a,n1b,nd2a,nd2b,
     *                   j2,r,dr,ds,xy,a0(1,md1a,2),a1(1,1,md1a,2),
     *                   aj(md1a,2),mask(nd1a,j2),w,w1(1,md1a,1,2),
     *                   iorder)
        else
          call cmpslopeMG (m,nd1a,nd1b,md1a,md1b,n1a,n1b,nd2a,nd2b,
     *                     j2,r,dr,ds,xy,a0(1,md1a,2),a1(1,1,md1a,2),
...
c
              if (m.lt.0) then
                do i=1,m
                  wsavel(i)=w1(i,j1,4,1)
                  wsaver(i)=w1(i,j1,3,2)
                end do
              end if
     ...
     *                      w1(1,j1,3,2),fl,fr,almax(2),method)
              ...
              if (m.lt.0) then
c                if (j1.eq.n1a.and.dr.gt.1.d-6) then
                if (dr.gt.1.d-6) then
                  methodMG=0
                  linsc=0
                  call cmpfluxMG (m,aj0,a20,a21,a22,wsavel,wsaver,
     *                            flchk,frchk,spchk,methodMG,linsc)
                  errfl=0.d0
                  errfr=0.d0
                  do i=1,9
                    errfl=max(dabs(fl(i)-flchk(i)),errfl)
                    errfr=max(dabs(fr(i)-frchk(i)),errfr)
                  end do
                  errwl=0.d0
                  errwr=0.d0
                  do i=1,3
                    iw=icw(i)
                    errwl=max(dabs(w1(iw,j1,4,1)-wsavel(iw)),errwl)
                    errwr=max(dabs(w1(iw,j1,3,2)-wsaver(iw)),errwr)
                  end do
                  if (max(errfl,errfr,errwl,errwr).gt.chktol) then
                    write(66,438)j1,j2,r,errfl,errfr,errwl,errwr
  438               format(1x,i3,1x,i3,1x,f15.8,4(1x,1pe9.2))
c                    do i=1,3
c                      iw=icw(i)
c                      wldif=wsavel(iw)-w1(iw,j1,4,1)
c                      wrdif=wsaver(iw)-w1(iw,j1,3,2)
c                      write(6,788)iw,wsavel(iw),w1(iw,j1,4,1),wldif,
c     *                               wsaver(iw),w1(iw,j1,3,2),wrdif
c  788                 format(1x,i1,2(2(1x,1pe15.8),1x,1pe9.2))
c                    end do
c                    read(5,*)idummy
                  end if
                  if (m.lt.0.and.j1.eq.n1a.and.j2.eq.1) then
                    do i=1,9
                      fldif=flchk(i)-fl(i)
                      frdif=frchk(i)-fr(i)
                      write(6,788)i,fl(i),flchk(i),fldif,
     *                              fr(i),frchk(i),frdif
  788                 format(1x,i1,2(2(1x,1pe15.8),1x,1pe9.2))
                    end do
                    read(5,*)idummy
                  end if
                end if
              ...
              if (m.lt.0) then
                if (j1.eq.n1a.and.dr.gt.1.d-3) then
c                 write(66,432)j1,r
c  432            format(1x,i3,1x,f15.8)
                  call chkfluxMG (m,aj0,a20,a21,a22,wsavel,wsaver,
     *                            w1(1,j1,4,1),w1(1,j1,3,2),fl,fr,
     *                            almax(2),method,errfl,errfr,
     *                            errwl,errwr)
                  if (max(errfl,errfr,errwl,errwr).gt.1.d-8) then
                    write(66,437)j1,r,errfl,errfr,errwl,errwr
  437               format(1x,i3,1x,f15.8,4(1x,1pe9.2))
                  end if
                end if
              end if
...
c new flux
              call cmpfluxMG (m,aj0,a20,a21,a22,w1(1,j1,4,1),
     *                        w1(1,j1,3,2),fl,fr,almax(2),
     *                        methodMG,linsc)
...
c              end if
     ...
     *                        w1(1,j1p1,1,1),fl,fr,almax(1),method)
                ...
                if (m.lt.0) then
c                  if (j2.eq.n2a.and.dr.gt.1.d-6) then
                  if (dr.gt.1.d-6) then
                    methodMG=0
                    linsc=0
                    call cmpfluxMG (m,aj0,a10,a11,a12,wsavel,wsaver,
     *                              flchk,frchk,spchk,methodMG,linsc)
                    errfl=0.d0
                    errfr=0.d0
                    do i=1,9
                    ...
                    do i=1,3
                      iw=icw(i)
                      errwl=max(dabs(w1(iw,j1  ,2,1)-wsavel(iw)),errwl)
                      errwr=max(dabs(w1(iw,j1p1,1,1)-wsaver(iw)),errwr)
                    end do
                    if (max(errfl,errfr,errwl,errwr).gt.chktol) then
                      write(66,439)j1,j2,r,errfl,errfr,errwl,errwr
  439                 format(1x,i3,1x,i3,1x,f15.8,4(1x,1pe9.2))
c                      if (m.lt.0.and.j1.eq.1.and.j2.eq.n2a) then
c                        do i=1,9
c                          fldif=flchk(i)-fl(i)
c                          frdif=frchk(i)-fr(i)
c                          write(6,789)i,fl(i),flchk(i),fldif,
c     *                                  fr(i),frchk(i),frdif
c  789                     format(1x,i1,2(2(1x,1pe15.8),1x,1pe9.2))
c                        end do
c                        read(5,*)idummy
c                      end if
                    end if
                ...
                if (m.lt.0) then
                  if (j2.eq.n2a.and.dr.gt.1.d-3) then
c                    write(66,432)j1,r
c  432               format(1x,i3,1x,f15.8)
                    call chkfluxMG (m,aj0,a10,a11,a12,wsavel,wsaver,
     *                              w1(1,j1,2,1),w1(1,j1p1,1,1),fl,fr,
     *                              almax(1),method,errfl,errfr,
     *                              errwl,errwr)
                    if (max(errfl,errfr,errwl,errwr).gt.1.d-10) then
                      write(66,436)j1,r,errfl,errfr,errwl,errwr
  436                 format(1x,i3,1x,f15.8,4(1x,1pe9.2))
                    end if
                  end if
                end if
...
c new flux
                call cmpfluxMG (m,aj0,a10,a11,a12,w1(1,j1,2,1),
     *                          w1(1,j1p1,1,1),fl,fr,almax(1),
     *                          methodMG,linsc)
...
c..add free stream correction to up (if a non-Cartesian grid)
          if (icart.eq.0) then
            if (imgeos.eq.0) then
              call cmpfree (m,nd1a,nd1b,md1a,md1b,n1a,n1b,
     *                      nd2a,nd2b,dr,ds,j2,rx,det,rx2,det2,
     *                      up,mask(nd1a,j2),w1(1,md1a,5,1),move)
            else
              call cmpfreeMG (m,nd1a,nd1b,md1a,md1b,n1a,n1b,
     *                        nd2a,nd2b,dr,ds,j2,rx,det,rx2,det2,
     *                        up,mask(nd1a,j2),w1(1,md1a,5,1),move)
            end if
          end if
...
c..add in TZ forcing (if necessary)
      if (itz.ne.0) then
        ...
        if (imgeos.eq.0) then
          ...
          do j2=n2a,n2b
            if (mask(j1,j2).ne.0) then
              call cmphtz (m,xy(j1,j2,1),xy(j1,j2,2),t,htz,iprim)
              do i=1,m
          ...
          do j2=n2a,n2b
            if (mask(j1,j2).ne.0) then
              call cmphtzMG (m,xy(j1,j2,1),xy(j1,j2,2),t,htz,iprim)
              do i=1,m
                up(j1,j2,i)=up(j1,j2,i)+dr*htz(i)
              end do
            end if
...
c
c      if (dabs(r).lt.1.d-6) then
c        if (ixy.eq.1) then
c          write(6,*)'Printing x-data...'
c          j2=6
c          do i=1,m
c            do j1=n1a-1,n1b+1
c              write(55,246)i,j1,(w1c(i,j1,j2,k),k=1,5)
c  246         format(2(1x,i3),5(1x,1pe15.8))
c            end do
c          end do
c        else
c          write(6,*)'Printing y-data...'
c          j1=6
c          do i=1,m
c            do j2=n2a-1,n2b+1
c              write(56,246)i,j2,(w1c(i,j1,j2,k),k=1,5)
cc  246         format(2(1x,i3),5(1x,1pe15.8))
c            end do
c          end do
c        end if
cc        read(5,*)ians
...
c++++++++++++++++++++
c
      subroutine cmphtz (m,x,y,t,htz,iprim)
c
...
c
      ...
      parameter (mlocal=20)
      dimension htz(m)
      dimension wt(mlocal),wx(mlocal),wy(mlocal),w0(mlocal)
      common / gasdat / gam(2),gm1(2),gp1(2),em(2),ep(2),ps0,bgas
      common / tzflow / eptz,itz
c
      if (m.gt.mlocal) then
        write(6,*)'Error (cmphtz) : m.gt.mlocal'
        stop
      end if
c
...
c
c use T=p/rho to convert to p, i.e. p'=rho*T'+rho'*T and then p=rho*T
      wt(4)=w0(1)*wt(4)+wt(1)*w0(4)
      wx(4)=w0(1)*wx(4)+wx(1)*w0(4)
      wy(4)=w0(1)*wy(4)+wy(1)*w0(4)
      w0(4)=w0(1)*w0(4)
      wt(8)=w0(5)*wt(8)+wt(5)*w0(8)
      wx(8)=w0(5)*wx(8)+wx(5)*w0(8)
      wy(8)=w0(5)*wy(8)+wy(5)*w0(8)
      w0(8)=w0(5)*w0(8)
c
c variables
      rs=w0(1)
...
c sound speed squared
      cs2=gam(1)*(ps+ps0)/rs
      cg2=gam(2)*(1.d0+bgas*rg*(1.d0-bgas*rg
     *                   /(gam(2)*(1.d0+bgas*rg))))*pg/rg
c
c TZ forcings in primitive form
      ...
      dvfact=(vg-vs)/ag
      do i=1,4
        htz(i)=wt(i)+us*wx(i)+vs*wy(i)
      end do
      htz(1)=htz(1)+rs*(wx(2)+wy(3))
      htz(2)=htz(2)+wx(4)/rs-dpfact*wx(9)
      htz(3)=htz(3)+wy(4)/rs-dpfact*wy(9)
      htz(4)=htz(4)+rs*cs2*(wx(2)+wy(3))
      do i=5,8
        htz(i)=wt(i)+ug*wx(i)+vg*wy(i)
      end do
      htz(5)=htz(5)+rg*(wx(6)+wy(7)-dufact*wx(9)-dvfact*wy(9))
      htz(6)=htz(6)+wx(8)/rg
      htz(7)=htz(7)+wy(8)/rg
      htz(8)=htz(8)+rg*cg2*(wx(6)+wy(7)-dufact*wx(9)-dvfact*wy(9))
      htz(9)=wt(9)+us*wx(9)+vs*wy(9)
c
c Axisymmetric contribution (leftover from cmfdu, not implemented)
c      ymin=1.d-8
c      if (iaxi.ne.0) then
c        if (dabs(y).gt.ymin) then
c          vy=v/y
c        else
c          vy=wy(3)
c        end if
c        htz(1)=htz(1)+rho*vy
c        htz(4)=htz(4)+rho*c2*vy
c      end if
c
c TZ forcings in the conservative form (if necessary)
      if (iprim.eq.0) then
        fact=0.5d0*(us**2+vs**2)+compac(as,0)
        rhoEs=(ps+gam(1)*ps0)/gm1(1)+rs*fact
        htz(4)=rhoEs*htz(9)+as*(htz(4)/gm1(1)+fact*htz(1)
     *          +rs*(us*htz(2)+vs*htz(3)+compac(as,1)*htz(9)))
        htz(1)=rs*htz(9)+as*htz(1)
        htz(2)=us*htz(1)+as*rs*htz(2)
        htz(3)=vs*htz(1)+as*rs*htz(3)
        fact=0.5d0*(ug**2+vg**2)
        rfact=1.d0/(1.d0+bgas*rg)
        rhoEg=pg*rfact/gm1(2)+rg*fact
        htz(8)=-rhoEg*htz(9)+ag*(rfact*(htz(8)-bgas*pg*rfact*htz(5))
     *           /gm1(2)+fact*htz(5)+rg*(ug*htz(6)+vg*htz(7)))
        htz(5)=-rg*htz(9)+ag*htz(5)
        htz(6)= ug*htz(5)+ag*rg*htz(6)
        htz(7)= vg*htz(5)+ag*rg*htz(7)
      end if
c
      return
      end
...
c++++++++++++++++++++
c
      subroutine cmphtzMG (m,x,y,t,htz,iprim)
c
c compute tz forcing function (for hydro part), MG EOS version
c
      implicit real*8 (a-h,o-z)
      parameter (mlocal=20)
      dimension htz(m)
...
c
      if (m.gt.mlocal) then
        write(6,*)'Error (cmphtzMG) : m.gt.mlocal'
        stop
      end if
c
c      write(76,100)x,y,t
...
c sound speed squared
      Vols=1.d0/rs
      cs2=c2mg(kmat(1),Vols,ps)
      Volg=1.d0/rg
      cg2=c2mg(kmat(2),Volg,pg)
c
c TZ forcings in primitive form
      dpfact=(pg-ps)/(as*rs)
      dufact=(ug-us)/ag
...
c solid variables
        j=1
        ideriv=1
        call energymg (j,rs,ps,ens,ensr,ensp,ideriv)
        Es=ens+0.5d0*(us**2+vs**2)+compac(as,0)
        htz(4)=rs*Es*htz(9)+as*Es*htz(1)
     *          +as*rs*(ensr*htz(1)+ensp*htz(4)
     *                  +us*htz(2)+vs*htz(3)+compac(as,1)*htz(9))
        htz(1)=rs*htz(9)+as*htz(1)
        htz(2)=us*htz(1)+as*rs*htz(2)
        htz(3)=vs*htz(1)+as*rs*htz(3)
c
c gas variables
        j=2
        ideriv=1
        call energymg (j,rg,pg,eng,engr,engp,ideriv)
        Eg=eng+0.5d0*(ug**2+vg**2)
        htz(8)=-rg*Eg*htz(9)+ag*Eg*htz(5)
     *           +ag*rg*(engr*htz(5)+engp*htz(8)
     *                   +ug*htz(6)+vg*htz(7))
        htz(5)=-rg*htz(9)+ag*htz(5)
        htz(6)= ug*htz(5)+ag*rg*htz(6)
        htz(7)= vg*htz(5)+ag*rg*htz(7)
...
c+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c
      subroutine cmppvsMG (m,nd1a,nd1b,n1a,n1b,nd2a,nd2b,j2,u,w)
c
c convert conservative u to primitive variables w, MG EOS version
c
      implicit real*8 (a-h,o-z)
      dimension u(nd1a:nd1b,nd2a:nd2b,m),w(m,n1a:n1b)
      dimension ifix(3)
      logical icorr,iprtchg
      common / matdatmg / kmat(2)
      data amin,amax / 1.d-12, 0.999999999999d0 /
      data rmin / 1.d-8 /
      data icorr,iprtchg / .true.,.true. /
c
      if (icorr) then
        do i=1,3
          ifix(i)=0
...
c..solid volume fraction
        w(9,j1)=u(j1,j2,9)
        if (w(9,j1).lt.amin.or.w(9,j1).gt.amax) then
          if (icorr) then
            w(9,j1)=min(amax,max(amin,w(9,j1)))
            ifix(1)=ifix(1)+1
            ichg=1
          else
            write(6,*)'Error (cmppvsMG) : alpha(solid) out of bounds'
            stop
          end if
        end if
c
c..solid density
        ...
        if (w(1,j1).le.rmin) then
          if (icorr) then
            ...
            write(6,*)'Error (cmppvsMG) : rho_solid<rmin0'
            stop
          end if
        end if
c
...
c..solid pressure
        w(4,j1)=u(j1,j2,4)/u(j1,j2,1)
        energy=w(4,j1)-.5d0*(w(2,j1)**2+w(3,j1)**2)-compac(w(9,j1),0)
        Vols=1.d0/w(1,j1)
        As=AmgV(kmat(1),Vols)
        Bs=BmgV(1,Vols,ierb)
        if (ierb.ne.0) then
          write(6,*)'Error (cmppvsMG) : cannot compute Bs'
          stop
        end if
        w(4,j1)=(energy-Bs)/As
c
c..gas density
        ...
        if (w(5,j1).le.rmin) then
          if (icorr) then
            ...
            write(6,*)'Error (cmppvsMG) : rho_gas<rmin'
            stop
          end if
        end if
c
...
c..gas pressure
        w(8,j1)=u(j1,j2,8)/u(j1,j2,5)
        energy=w(8,j1)-.5d0*(w(6,j1)**2+w(7,j1)**2)
        Volg=1.d0/w(5,j1)
        Ag=AmgV(kmat(2),Volg)
        Bg=BmgV(2,Volg,ierb)
        if (ierb.ne.0) then
          write(6,*)'Error (cmppvsMG) : cannot compute Bg'
          stop
        end if
        w(8,j1)=(energy-Bg)/Ag
c
...
c
c..advected variables, as needed
      if (m.gt.9) then
        do j1=n1a,n1b
          do i=10,m
            w(i,j1)=u(j1,j2,i)
...
c
      if (ichg.ne.0) then
        if (iprtchg) then
          icnt=ifix(1)+ifix(2)+ifix(3)
          write(6,100)icnt,(ifix(i),i=1,3)
  100     format(' Warning (cmppvsMG) : total solution changes =',i6,/,
     *           '                          **    alpha(solid) =',i6,/,
     *           '                          **  density(solid) =',i6,/,
     *           '                          **  density( gas ) =',i6)
        end if
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c
      subroutine cmpslope (m,nd1a,nd1b,md1a,md1b,n1a,n1b,nd2a,nd2b,j2,
...
c
     ...
     *          w(m,md1a:md1b,3),w1(m,md1a:md1b,5)
      dimension dw(9),b(9,2),al(9),fact(5),htz(9)
      logical isolid,igas,ilimit
      common / gasdat / gam(2),gm1(2),gp1(2),em(2),ep(2),ps0,bgas
      common / tzflow / eptz,itz
      common / cutdat / abmin1
...
c
          rho=w(5,j1,2)
          bfact=1.d0+bgas*rho
          gp=w(8,j1,2)*(gam(2)+bgas*rho*(gm1(2)*bfact+1.d0)/bfact)
          c2=gp/rho
          if (c2.lt.c2min) then
...
c..supply TZ forcing to the primitive state (if necessary)
          if (itz.ne.0) then
            iprim=1
            call cmphtz (m,xy(j1,j2,1),xy(j1,j2,2),r,htz,iprim)
            do k=1,5
              do i=1,m
...
c              if (b(i,1)*b(i,2).gt.0.d0.and.m.gt.0) then

c             ijunk(i,j1)=1

                beta=dsign(1.d0,b(i,1))*dmin1(dabs(b(i,1)),
     *                                        dabs(b(i,2)))
                ...
                fact(5)=fact(3)
                do k=1,5
                  w1(1,j1,k)=w1(1,j1,k)-fact(k)*rhob
...
c             ijunk(i,j1)=1
     ...
     *                                        dabs(b(i,2)))
                ...
                fact(5)=fact(3)
                do k=1,5
                  w1(5,j1,k)=w1(5,j1,k)-fact(k)*rho
...
c             ijunk(i,j1)=1
     ...
     *                                      dabs(b(i,2)))
              ...
              if (igas) then
                do k=1,5
                  w1(5,j1,k)=w1(5,j1,k)+fact(k)*dvda*rho*dv
          ...
          if (isolid.and.m.gt.9) then
            if (ilimit) then
              ...
              do i=10,m
                ...
                b2=w(i,j1+1,2)-w(i,j1,2)
                beta=0.5d0*(b1+b2)
                fact(1)=(al(2)+.5d0)*beta
                fact(2)=(al(2)-.5d0)*beta
                fact(3)=al(2)*beta
                fact(4)=fact(3)
                fact(5)=fact(3)
                do k=1,5
          ...
          if (isolid.and.m.gt.9) then
            if (ilimit) then
              ...
              do i=10,m
                ...
                fact(1)=al(2)*beta
                fact(2)=fact(1)
                fact(3)=(al(2)+.5d0)*beta
                fact(4)=(al(2)-.5d0)*beta
                fact(5)=fact(1)
                do k=1,5
                  w1(i,j1,k)=w1(i,j1,k)+fact(k)
                end do
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c
      subroutine cmpslopeMG (m,nd1a,nd1b,md1a,md1b,n1a,n1b,nd2a,nd2b,
     *                       j2,r,dr,ds,xy,a0,a1,aj,mask,w,w1,iorder)
c
c slope correction  (MG version)
c
      implicit real*8 (a-h,o-z)
      dimension ds(2),xy(nd1a:nd1b,nd2a:nd2b,2),a0(2,md1a:md1b),
     *          a1(2,2,md1a:md1b),aj(md1a:md1b),mask(nd1a:nd1b),
     *          w(m,md1a:md1b,3),w1(m,md1a:md1b,5)
      dimension dw(9),b(9,2),al(9),alm(9),alp(9),fact(5),htz(9)
      logical isolid,igas,ilimit,iupwind
      common / matdatmg / kmat(2)
      common / tzflow / eptz,itz
      common / cutdat / abmin1
      data amin,amax,c2min,abtiny / 1.d-2, 0.99d0, 1.d-6, 1.d-8 /
...
c
          ...
          if (w(9,j1,2).gt.amax) igas=.false.
c
c solid density and sound speed (rhob,cb)
          rhob=w(1,j1,2)
          Volb=1.d0/rhob
          cb2=c2mg(kmat(1),Volb,w(4,j1,2))
          if (cb2.lt.c2min) then
            write(6,*)'Error (cmpslopeMG) : cb2<c2min'
            stop
          end if
          cb=dsqrt(cb2)
          gpb=rhob*cb2
c
c gas density and sound speed (rho,c)
          rho=w(5,j1,2)
          Vol=1.d0/rho
          c2=c2mg(kmat(2),Vol,w(8,j1,2))
          if (c2.lt.c2min) then
            write(6,*)'Error (slope) : c2<c2min'
            stop
          end if
          c=dsqrt(c2)
          gp=rho*c2
...
c..supply TZ forcing to the primitive state (if necessary)
          if (itz.ne.0) then
            iprim=1
            call cmphtzMG (m,xy(j1,j2,1),xy(j1,j2,2),r,htz,iprim)
            do k=1,5
              do i=1,m
                w1(i,j1,k)=w1(i,j1,k)+dr2*htz(i)
              end do
...
c
          if (iupwind) then
            do i=1,9
              alm(i)=min(al(i),0.d0)+.5d0
              alp(i)=max(al(i),0.d0)-.5d0
            end do
          else
            do i=1,9
              alm(i)=al(i)+.5d0
...
c
          do k=1,4
            if (w1(1,j1,k).le.0.d0.or.w1(5,j1,k).le.0.d0.or.
     *          w1(4,j1,k).le.0.d0.or.w1(8,j1,k).le.0.d0) then
              do i=1,m
                w1(i,j1,k)=w(i,j1,2)
              end do
            else
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c
      subroutine cmpfreeMG (m,nd1a,nd1b,md1a,md1b,n1a,n1b,nd2a,nd2b,
     *                      dr,ds,j2,rx,det,rx2,det2,up,mask,w1,move)
c
c free-stream correction for non-Cartesian grids (MG version)
c
      implicit real*8 (a-h,o-z)
      dimension rx(nd1a:nd1b,nd2a:nd2b,2,2),det(nd1a:nd1b,nd2a:nd2b),
     *          rx2(nd1a:nd1b,nd2a:nd2b,2,2),det2(nd1a:nd1b,nd2a:nd2b),
...
c solid total enthalpy
            ...
            enthalpy=(As*w1(4,j1)+Bs)+Vols*w1(4,j1)
            hs=w1(1,j1)*(enthalpy+compac(as,0))
     *         +.5d0*(rus*w1(2,j1)+rvs*w1(3,j1))
c
c gas total enthalpy
            Volg=1.d0/w1(5,j1)
            Ag=AmgV(kmat(2),Volg)
            Bg=BmgV(2,Volg,ierb)
            if (ierb.ne.0) then
            ...
            enthalpy=(Ag*w1(8,j1)+Bg)+Volg*w1(8,j1)
            hg=w1(5,j1)*enthalpy
     *         +.5d0*(rug*w1(6,j1)+rvg*w1(7,j1))
c
c average of the determinant
...
c
c this is the original version, probably used to verify correctness
c with the flux routines in dudr_mp1dv.f 
c
...
c+++++++++++++++++++++++++++++++++++++++++++++++++++
c
      subroutine chkfluxMG (m,aj,a0,a1,a2,wl,wr,wlout,wrout,fl,fr,
     *                      speed,method,errfl,errfr,errwl,errwr)
c
c probably only works if m=9, i.e. no advected scalars (I did not try
c to modify this subroutine for the case m>9)
c
      ...
      common / matdatmg / kmat(2)
c
c component-map for x direction
      data ic / 9, 1, 2, 4, 5, 6, 8 /
c
c component-map for y direction
c      data ic / 9, 1, 3, 4, 5, 7, 8 /
c
      kmat(1)=4   ! stiffened gas
      kmat(2)=5   ! virial gas
c
c      write(6,*)'here i am'
c      read(5,*)idum
...
c
c      if (errwl.gt.1.d-14) then
c      if (max(errfl,errfr).gt.1.d-4) then
c        write(6,*)'writing gdflux.out ...'
c        open (40,file='gdflux.out')
c        write(40,400)m,(wl(i),i=1,m),(wr(i),i=1,m)
c  400   format(1x,i2,2(/,9(1x,1pe22.15)))
c        do i=1,2
c          write(40,401)gam(i),gm1(i),gp1(i),em(i),ep(i)
c  401     format(5(1x,1pe22.15))
c        end do
c        write(40,402)ps0,bgas,abmin1,method,aj,a0,a1,a2,
c     *               cfact1,cfact2,heat
c  402   format(1x,3(1pe22.15,1x),i2,/,4(1x,1pe22.15),/,
c     *         3(1x,1pe22.15))
c        stop
c      end if
c
...
c check whether to accept the linearized jump solution
        if (lcont.eq.0) then
c
c          write(6,*)'lcont =',lcont
c          write(6,*)'Enter kstop : (0=cont, 1=stop)'
c          read(5,*)kstop
c          if (kstop.ne.0) stop
c
...
c      write(6,345)(g(i),i=1,4)
c  345 format('** g=',4(/,1x,1pe22.15))
c      pause
c
c scale residual
      resid=0.d0
...
c
c update
c        write(6,320)it,rm(1),g(1),rm(2),g(2),
c     *              pm(1,1),g(3),pm(2,1),g(4)
c  320   format(' it =',i2,4(/,4x,2(1x,1pe15.8)))
c        write(6,*)'vm=',vm(1,2),vm(2,2)
c
        rm(1)=rm(1)-g(1)
        rm(2)=rm(2)-g(2)
        pm(1,1)=pm(1,1)-g(3)
