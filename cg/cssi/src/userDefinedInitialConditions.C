...
enum UserDefinedInitialConditionOptions
{
  ...
  planarSmoothInterfaceWithShock,
  circularSmoothInterfaceWithShock,
  circularSmoothInterfaceWithShockM1p2,
  circularSmoothInterfaceWithShockM1p2Rxn,
  circularSmoothInterfaceWithShockM1p4,
  circularSmoothInterfaceWithShockM1p6,
  circularSmoothInterfaceWithShock2,
  longEllipticalSmoothInterfaceWithShock,
  tallEllipticalSmoothInterfaceWithShock,
  tallEllipticalSmoothInterfaceWithShockM1p6,
  bubblesShock,
  compliantCorner,
  compliantCornerDES,
  makeFail,
...
//                ug(i1,i2,i3,sc+1)=frac*gammaSolid*piSolid/(gammaSolid-1.);
      ...
      else if( option == planarSmoothInterfaceWithShock )
      {
        c.update(MappedGrid::THEvertex);  // make sure the vertex array has been created
//        realArray & vertex = c.vertex();  // grid points

        real p0=1., M0=2., Pi=3.141592653;
        real xShock=.2, xWedge=.3, wedgeAngle=45.*Pi/180.;
        real gammaGas=1.4, rhoGas=1.4;
        real gammaSolid=5., piSolid=6857.8, rhoSolid=2296.68705;   // these are the new values
//        real gammaSolid=5., piSolid=6843.3, rhoSolid=2290.;  These are old values
...
//        ug(I1,I2,I3,sc+1)=(gammaSolid*piSolid/(gammaSolid-1.))*phi;
      ...
      else if( option == circularSmoothInterfaceWithShockM1p2 )
      {
        c.update(MappedGrid::THEvertex);  // make sure the vertex array has been created
//        realArray & vertex = c.vertex();  // grid points
...
//        ug(I1,I2,I3,sc+1)=(gammaSolid*piSolid/(gammaSolid-1.))*phi;
      ...
      else if( option == circularSmoothInterfaceWithShockM1p2Rxn )
      {
        c.update(MappedGrid::THEvertex);  // make sure the vertex array has been created
//        realArray & vertex = c.vertex();  // grid points
...
//        real gammaSolid=5., piSolid=6843.3, rhoSolid=2290.;  These are old values
        ...
        real gammaSolid2=3., piSolid2=0.;   // values for the solid reaction products
        real heatSolid=-44.;                // solid heat release

        real cvSolid=1.5, cvGas=0.717625;
        real mu1Solid=1./(gammaSolid-1.);
        real mu1Solid2=1./(gammaSolid2-1.);
        real mu2Solid=gammaSolid*piSolid*mu1Solid;
        real mu2Solid2=gammaSolid2*piSolid2*mu1Solid2;
        real mu1Gas=1./(gammaGas-1.);
        real T0=p0*mu1Gas/(rhoGas*cvGas);
...
//        real sig=24000.;  // this value is grid dependent for h=(1/4000)/4
//        real sig=6000.;  // this value is grid dependent for h=(1/4000)
//        real sig=1500.;  // this value is grid dependent for h=4*(1/4000)
//        real sig=375.;  // this value is grid dependent for h=16*(1/4000)
        real sig=300;
...
//        ug(I1,I2,I3,sc+1)=(gammaSolid*piSolid/(gammaSolid-1.))*phi;
        ...
        ug(I1,I2,I3,uc)=0.;                                                     // velocity is zero
        ug(I1,I2,I3,vc)=0.;
        ug(I1,I2,I3,sc)=0.;                                                     // lambda=0
        ug(I1,I2,I3,sc+1)=mu1Solid*phi+mu1Gas*(1.-phi);                         // set mu1 and mu2 according to phi
        ug(I1,I2,I3,sc+2)=mu1Solid2*phi+mu1Gas*(1.-phi);
        ug(I1,I2,I3,sc+3)=mu2Solid*phi;
        ug(I1,I2,I3,sc+4)=mu2Solid2*phi;
        ug(I1,I2,I3,sc+5)=heatSolid*phi;

        cvMix=(1.-phi)*cvGas+phi*cvSolid;                                       // mixture Cv
        ug(I1,I2,I3,rc)=(ug(I1,I2,I3,sc+1)*p0+ug(I1,I2,I3,sc+3))/(T0*cvMix);    // set rho so that mixture temperature=T0
        ug(I1,I2,I3,tc)=p0/ug(I1,I2,I3,rc);
      ...
      else if( option == circularSmoothInterfaceWithShockM1p4 )
      {
        c.update(MappedGrid::THEvertex);  // make sure the vertex array has been created
//        realArray & vertex = c.vertex();  // grid points
...
//        ug(I1,I2,I3,sc+1)=(gammaSolid*piSolid/(gammaSolid-1.))*phi;
      ...
      else if( option == circularSmoothInterfaceWithShockM1p6 )
      {
        c.update(MappedGrid::THEvertex);  // make sure the vertex array has been created
//        realArray & vertex = c.vertex();  // grid points
...
//        realArray & vertex = c.vertex();  // grid points
        ...
        real p0=1., M0=2.;
// old        real xShock=.2, xCenter=.5, rInterface=.2;
        real xShock=-.4175, xCenter=.0, rInterface=.3175;
        real gammaGas=1.4, rhoGas=1.4;
        real gammaSolid=5., piSolid=6857.8, rhoSolid=2296.68705;   // these are the new values
//        real gammaSolid=5., piSolid=6843.3, rhoSolid=2290.;  These are old values
	...
	RealArray radius;
	radius = sqrt( SQR(vertex(I1,I2,I3,axis1)-xCenter) + 4.*SQR(vertex(I1,I2,I3,axis2)) );

        real sig=48000.;  // this value is grid dependent for h=(1/4000)/4  (extra sharp interface)
//        real sig=24000.;  // this value is grid dependent for h=(1/4000)/4
//        real sig=12000.;  // this value is grid dependent for h=(1/4000)  (extra sharp interface)
//        real sig=6000.;  // this value is grid dependent for h=(1/4000)
	RealArray phi, cvMix;
	phi = .5 + .5*tanh(sig*(radius-rInterface));
...
//        realArray & vertex = c.vertex();  // grid points
        ...
        real p0=1., M0=2.;
// old        real xShock=.2, xCenter=.5, rInterface=.2;
        real xShock=-.2260, xCenter=.0, rInterface=.1260;
        real gammaGas=1.4, rhoGas=1.4;
        real gammaSolid=5., piSolid=6857.8, rhoSolid=2296.68705;   // these are the new values
//        real gammaSolid=5., piSolid=6843.3, rhoSolid=2290.;  These are old values
...
//        ug(I1,I2,I3,sc+1)=(gammaSolid*piSolid/(gammaSolid-1.))*phi;
      ...
      else if( option == tallEllipticalSmoothInterfaceWithShockM1p6 )
      {
        c.update(MappedGrid::THEvertex);  // make sure the vertex array has been created
//        realArray & vertex = c.vertex();  // grid points
...
//==============================================================================================
{
  ...
  aString menu[]=  
  {
    ...
    "planar (smooth) interface with shock",
    "circular (smooth) interface with shock",
    "circular (smooth) interface with shock M=1.2",
    "circular (smooth) interface with shock M=1.4",
    "circular (smooth) interface with shock M=1.6",
    ...
    "tall elliptical (smooth) interface with shock M=1.6",
    "bubble with shock",
    "rate stick",
    "PDE thruster",
    "PDE thruster 3d",
...
//      sScanF(answer,"%e",&tGradient);  
    ...
    else if( answer=="planar (smooth) interface with shock" )
    {
      option=planarSmoothInterfaceWithShock;
    }
    else if( answer=="circular (smooth) interface with shock" )
    {
      option=circularSmoothInterfaceWithShock;
    }
    else if( answer=="circular (smooth) interface with shock M=1.2" )
    {
      option=circularSmoothInterfaceWithShockM1p2;
    }
    else if( answer=="circular (smooth) interface with shock M=1.2 Reactive" )
    {
      option=circularSmoothInterfaceWithShockM1p2Rxn;
    }
    else if( answer=="circular (smooth) interface with shock M=1.4" )
    {
      option=circularSmoothInterfaceWithShockM1p4;
    }
    else if( answer=="circular (smooth) interface with shock M=1.6" )
    {
      option=circularSmoothInterfaceWithShockM1p6;
    }
    ...
    else if( answer=="tall elliptical (smooth) interface with shock M=1.6" )
    {
      option=tallEllipticalSmoothInterfaceWithShockM1p6;
    }
