...
c                tang .dot. { F, G }^T gets put into teq
                  ...
                  do m1=-hwidth, hwidth
                     do c=0,ncmp-1
                        ...
                        coeff_u = coeff(icf(m1,m2,m3,NEQ,c),i1,i2,i3)
                        coeff_v = coeff(icf(m1,m2,m3,TEQ,c),i1,i2,i3)
                        coeff(icf(m1,m2,m3,NEQ,c),i1,i2,i3) =   norm(1)*coeff_u + norm(2)*coeff_v
                        coeff(icf(m1,m2,m3,TEQ,c),i1,i2,i3) =   tang(1)*coeff_u + tang(2)*coeff_v
                     end do
                  end do
                  end do
                  end do
       #Else 
                  ...
                  rhs(i1,i2,i3,NEQ) = norm(1)*coeff_u + norm(2)*coeff_v
                  rhs(i1,i2,i3,TEQ) = tang(1)*coeff_u + tang(2)*coeff_v
       #End

#endMacro

#beginMacro SETCENTER(LOC)
    #If #LOC == "GHOST"
       i1 = i1g
       i2 = i2g
       i3 = i3g
...
c     INPUT
     ...
     &                       nd1a:nd1b,nd2a:nd2b,nd3a:nd3b) ! coefficient matrix
      ...
      double precision rsxy(nd1a:nd1b,nd2a:nd2b,nd3a:nd3b,0:nd-1,0:nd-1) ! metric derivatives
...
c     LOCAL
      ...
      double precision rr,aa(0:2,0:2),jdet,jdetg,rtmp,aainv(0:2,0:2),den
      double precision ubv(0:30),norm(3),tang(3),grav(3),scale,hlen,sint
      double precision flowsum,flowsum2,drar,jaci,av2,av4
      logical useneumanntemp,iscorner,useneumannvel,extrapponbdy,now
      logical usedirichletrhoinflow,iswall,skipslip,sliponwall,ghostrpos
      ...
      double precision rho,uvel,vvel,wvel,temp,mdot,port,pdotn,prhs,pfac
c     LOCAL PARAMETERS
...
c            if ( isaxi.gt.0 ) scale = scale/xyz(i1,i2,i3,2)
     ...
     &              (bc(0,1).eq.noslipwall.or.bc(0,1).eq.slipwall)
            else if(i1.eq.indexRange(0,0).and.i2.eq.indexRange(1,1))then
               iscorner = 
     &              (bc(0,0).eq.noslipwall.or.bc(0,0).eq.slipwall).and.
     &              (bc(1,1).eq.noslipwall.or.bc(1,1).eq.slipwall)
            else if(i1.eq.indexRange(1,0).and.i2.eq.indexRange(0,1))then
               iscorner = 
     &              (bc(1,0).eq.noslipwall.or.bc(1,0).eq.slipwall).and.
     &              (bc(0,1).eq.noslipwall.or.bc(0,1).eq.slipwall)
            else if(i1.eq.indexRange(1,0).and.i2.eq.indexRange(1,1))then
               iscorner = 
     &              (bc(1,0).eq.noslipwall.or.bc(1,0).eq.slipwall).and.
     &              (bc(1,1).eq.noslipwall.or.bc(1,1).eq.slipwall)
            endif
...
c                     if ( bc(side,axis).ne.subsonicinflow ) then
                     UVCF(   0,   0,   0, 1d0,rc,tc,COEFF)
                     UVCF( is1, is2, is3,-2d0,rc,tc,COEFF)
                     UVCF( 2*is1, 2*is2, 2*is3, 1d0,rc,tc,COEFF)
     ...
     &                    usedirichletrhoinflow ) then
                     ...
                     if ( .false. ) then
                        if ( rtmp.gt.0d0 .or. .true.) then
                           UVCF(-is1,-is2,-is3, 1d0,rc,tc,COEFF)
                           UVCF(   0,   0,   0,-2d0,rc,tc,COEFF)
                           UVCF( is1, is2, is3, 1d0,rc,tc,COEFF)
                        else
                           UVCF(-is1,-is2,-is3, 1d0,rc,tc,COEFF)
                           UVCF(   0,   0,   0,-1d0,rc,tc,COEFF)
                        ...
                        UVCF(-is1,  -is2, -is3, 1d0,rc,tc,COEFF)
                        UVCF(   0,     0,    0,-3d0,rc,tc,COEFF)
                        UVCF(  is1,  is2,  is3, 3d0,rc,tc,COEFF)
                        UVCF(2*is1,2*is2,2*is3,-1d0,rc,tc,COEFF)
                     else

                        coeff(icf(0,0,0,rc,rc),i1g,i2g,i3g) = 1d0
                        coeff(icf(2*is1,2*is2,2*is3,rc,rc),i1g,i2g,i3g)=-1d0
...
c              mixed bc on pressure for outflow
                  ...
                  UVCF(0,0,0,pfac,rc,tc,COEFF)
                  UUXC(1,pdotn*norm(1),rc,tc)
                  UUXC(1,pdotn*norm(1),tc,rc)
                  UUXC(2,pdotn*norm(2),rc,tc)
                  UUXC(2,pdotn*norm(2),tc,rc)
                  if ( .false. ) then
...
c                 temperature part of pressure gradient
     ...
     &                    ) then !usestrik ) then
                        alpha = rprm(18)
                        UX4MC((d-uc+1),vect(d)*gm2,tc)
                        alpha = 0d0
                     else
                        UXC((d-uc+1),vect(d)*gm2,tc)
                     endif   

                     if ( .false. ) then
c                 density part of pressure gradient
c     we linearize a discretized version of gm2*u[tc]*log(u[rc])_c
                        lur = (dlog(dabs(u(i1l+1,i2l,i3l,rc)))-dlog(dabs(u(i1l-1,i2l,i3l,rc))))
                        lus = (dlog(dabs(u(i1l,i2l+1,i3l,rc)))-dlog(dabs(u(i1l,i2l-1,i3l,rc))))
...
c     strikwerda-like correction term
                         UUX4MC(d,(vect(d)*gm2/u(i1l,i2l,i3l,rc)),tc,rc)
...
c     subtract off the part we don't need (done above with log(rho))
                          UUXC(d,(-vect(d)*gm2/u(i1l,i2l,i3l,rc)),tc,rc)
...
c     u[uc]_xx/uc[rc] linearization (will have RHS)
                  UXYC(1,1,(-vect(1)*f43*oren/u(i1l,i2l,i3l,rc)),uc)
                  coeff(icf(0,0,0,eqn,rc),i1,i2,i3) = 
     &                 coeff(icf(0,0,0,eqn,rc),i1,i2,i3) - 
...
c     u[uc]_yy/uc[rc] linearization (will have RHS)
                  UXYC(2,2,(-vect(1)*oren/u(i1l,i2l,i3l,rc)),uc)
                  coeff(icf(0,0,0,eqn,rc),i1,i2,i3) = 
     &                 coeff(icf(0,0,0,eqn,rc),i1,i2,i3) - 
...
c     u[vc]_xy/uc[rc] linearization (will have RHS)
                  UXYC(1,2,(-vect(1)*f13*oren/u(i1l,i2l,i3l,rc)),vc)
                  coeff(icf(0,0,0,eqn,rc),i1,i2,i3) = 
     &                 coeff(icf(0,0,0,eqn,rc),i1,i2,i3) - 
     &                 (-vect(1)*f13*oren*uxx(i1l,i2l,i3l,vc,1,2))/
     &                 (u(i1l,i2l,i3l,rc)**2)
                  
                  if ( isaxi.gt.0 .and. xyz(i1l,i2l,i3l,2).gt.0d0 ) then
                     
c     u[uc]_y/y/u[rc] linearization (will have RHS)
                     coeff(icf(0,0,0,eqn,rc),i1,i2,i3) = 
     &                    coeff(icf(0,0,0,eqn,rc),i1,i2,i3) +
     &                    vect(1)*oren*ux(i1l,i2l,i3l,uc,2)/xyz(i1l,i2l,i3l,2)/(u(i1l,i2l,i3l,rc)**2)

                     UXC(2,(-vect(1)*oren/xyz(i1l,i2l,i3l,2)/u(i1l,i2l,i3l,rc)),uc)

c     u[vc]_x/y/uc[rc] linearization (will have RHS)
                     coeff(icf(0,0,0,eqn,rc),i1,i2,i3) = 
     &                    coeff(icf(0,0,0,eqn,rc),i1,i2,i3) +
     &                    vect(1)*f13*oren*ux(i1l,i2l,i3l,vc,1)/xyz(i1l,i2l,i3l,2)/(u(i1l,i2l,i3l,rc)**2)

                     UXC(1,(-vect(1)*f13*oren/xyz(i1l,i2l,i3l,2)/u(i1l,i2l,i3l,rc)),vc)
...
c     u[vc]_yy/uc[rc] linearization (will have RHS)
                  UXYC(2,2,(-vect(2)*f43*oren/u(i1l,i2l,i3l,rc)),vc)
                  coeff(icf(0,0,0,eqn,rc),i1,i2,i3) = 
     &                 coeff(icf(0,0,0,eqn,rc),i1,i2,i3) -
...
c     u[vc]_xx/uc[rc] linearization (will have RHS)
                  UXYC(1,1,(-vect(2)*oren/u(i1l,i2l,i3l,rc)),vc)
                  coeff(icf(0,0,0,eqn,rc),i1,i2,i3) = 
     &                 coeff(icf(0,0,0,eqn,rc),i1,i2,i3) - 
...
c     u[uc]_xy/uc[rc] linearization (will have RHS)
                  UXYC(2,1,(-vect(2)*f13*oren/u(i1l,i2l,i3l,rc)),uc)
                  coeff(icf(0,0,0,eqn,rc),i1,i2,i3) = 
     &                 coeff(icf(0,0,0,eqn,rc),i1,i2,i3) -
...
c     u[vc]_y/y/u[rc] linearization (will have RHS)
     ...
     &                    (u(i1l,i2l,i3l,rc)**2)
                     ...
                     UXC(2,(-vect(2)*f43*oren/u(i1l,i2l,i3l,rc)/xyz(i1l,i2l,i3l,2)),vc)
...
c               the following section was uncommented 080110
     ...
     &                 -norm(2)!- because norm points out
               ...
               else if ( .false. .and. port.lt.1e-10 .and. bc(side,axis).eq.subsonicoutflow ) then 
c                 outflow condition with specified mass flux
!!! THIS SECTION NEVER GETS CALLED, IS SUBSONICOUTFLOW WITH MASS FLUX BROKEN??!!
                  eqn = neq
                  UVCF(0,0,0,norm(1),uc,rc,COEFF)
                  UVCF(0,0,0,norm(2),vc,rc,COEFF)

                  eqn = teq
                  UVCF(0,0,0,tang(1),uc,rc,COEFF)
                  UVCF(0,0,0,tang(2),vc,rc,COEFF)
...
c     boundary point 
                  ...
                  UVCF(is1,is2,is3,rr*aj(i1l,i2l,i3i)*rsxy(i1l,i2l,i3l,axis,0),rc,uc,COEFF)
                  UVCF(is1,is2,is3,rr*aj(i1l,i2l,i3i)*rsxy(i1l,i2l,i3l,axis,1),rc,vc,COEFF)
...
c     ghost ghost point
                     ...
                     UVCF(-is1,-is2,-is3,rr*aj(i1g-is1,i2g-is2,i3-is3)*rsxy(i1g-is1,i2g-is2,i3-is3,axis,0),rc,uc,COEFF)
                     UVCF(-is1,-is2,-is3,rr*aj(i1g-is1,i2g-is2,i3-is3)*rsxy(i1g-is1,i2g-is2,i3-is3,axis,1),rc,vc,COEFF)
c     ghost point
...
c               UVCF( 2*is1, 2*is2, 2*is3,-rr*aj(i1i,i2i,i3i)*rsxy(i1i,i2i,i3i,a,1),rc,vc,COEFF)
                  ...
                  coeff(icf(2*is1,2*is2,2*is3,teq,uc),i1,i2,i3)=-tang(1)
                  coeff(icf(2*is1,2*is2,2*is3,teq,vc),i1,i2,i3)=-tang(2)
...
c                   coeff(icf(2*is1,2*is2,2*is3,neq,vc),i1,i2,i3)=norm(2)
     ...
     &                    rr = rr*x(i1g,i2g,i3g,radaxis)
                     UVCF(0,0,0,rr*aj(i1g,i2g,i3g)*rsxy(i1g,i2g,i3g,a,0),rc,uc,COEFF)
                     UVCF(0,0,0,rr*aj(i1g,i2g,i3g)*rsxy(i1g,i2g,i3g,a,1),rc,vc,COEFF)
...
c                      neumann bc                        
                     ...
                     if ( isaxi.gt.0 .and. ghostrpos ) 
     &                    rr = rr*x(i1i,i2i,i3i,radaxis)               
                     UVCF( 2*is1, 2*is2, 2*is3,rr*aj(i1i,i2i,i3i)*rsxy(i1i,i2i,i3i,a,0),rc,uc,COEFF)
                     UVCF( 2*is1, 2*is2, 2*is3,rr*aj(i1i,i2i,i3i)*rsxy(i1i,i2i,i3i,a,1),rc,vc,COEFF)
                  endif

                  PIVOTTRICK(neq,teq,COEFF) !ghost point pivot trick
...
c     &                 ) then
                     ...
                     PIVOTTRICK(neq,teq,COEFF) ! boundary point pivot trick
                  endif
...
c                     if ( bc(side,axis).ne.subsonicinflow ) then
                     ...
                     UVCF(   0,   0,   0, 1d0,rc,tc,RHS)
                     UVCF( is1, is2, is3,-2d0,rc,tc,RHS)
                     UVCF( 2*is1, 2*is2, 2*is3, 1d0,rc,tc,RHS)
c                     endif
                     if ( bc(side,axis).eq.subsonicinflow .and.
     &                    usedirichletrhoinflow ) then
                        SETCENTER(BOUNDARY)
...
c                        print *,i1,i2,i3,rho
                     ...
                     if ( .false. ) then
                        if ( rtmp.gt.0d0 .or. .true.) then
                           UVCF(-is1,-is2,-is3, 1d0,rc,tc,RHS)
                           UVCF(   0,   0,   0,-2d0,rc,tc,RHS)
                           UVCF( is1, is2, is3, 1d0,rc,tc,RHS)
                        else
                           UVCF(-is1,-is2,-is3, 1d0,rc,tc,RHS)
                           UVCF(   0,   0,   0,-1d0,rc,tc,RHS)
                        ...
                        UVCF(-is1,  -is2, -is3, 1d0,rc,tc,RHS)
                        UVCF(   0,     0,    0,-3d0,rc,tc,RHS)
                        UVCF(  is1,  is2,  is3, 3d0,rc,tc,RHS)
                        UVCF(2*is1,2*is2,2*is3,-1d0,rc,tc,RHS)
                     else
...
c     (4 u[uc]_xx/3 + u[uc]_yy + u[vc]_xy/3 + axifac*(u[uc]_y/y + u[vc]_x/y/3))/u[rc]/ren
                     rhs(i1,i2,i3,eqn) = rhs(i1,i2,i3,eqn) +
     &                    vect(1)*oren*( f43*uxx(i1l,i2l,i3l,uc,1,1) + uxx(i1l,i2l,i3l,uc,2,2) + 
...
c     (4 u[vc]_yy/3 + u[vc]_xx + u[uc]_xy/3 + axifac*(4/3)*(u[vc]_y/y - u[vc]/y^2))/u[rc]/ren
                     rhs(i1,i2,i3,eqn) = rhs(i1,i2,i3,eqn) + 
     &                    vect(2)*oren*( f43*uxx(i1l,i2l,i3l,vc,2,2) + uxx(i1l,i2l,i3l,vc,1,1) +
     &                    f13*uxx(i1l,i2l,i3l,uc,2,1) )/u(i1l,i2l,i3l,rc)
                     
                     if ( isaxi.gt.0 .and. xyz(i1l,i2l,i3l,2).gt.0d0)then
                        rhs(i1,i2,i3,eqn) = rhs(i1,i2,i3,eqn) +
     &                       vect(2)*oren*(f43*(ux(i1l,i2l,i3l,vc,2)-u(i1l,i2l,i3l,vc)/xyz(i1l,i2l,i3l,2))/
     &                       (u(i1l,i2l,i3l,rc)*xyz(i1l,i2l,i3l,2)))
...
c                     print *,i1,i2,rhs(i1,i2,i3,uc),rhs(i1,i2,i3,vc)
     ...
     &                 tang(1)*rhs(i1,i2,i3,uc)+tang(2)*rhs(i1,i2,i3,vc)
               ...
               else if ( port.lt.1e-10 .and. bc(side,axis).eq.subsonicoutflow) then 
c                 outflow condition with specified mass flux
c                  print *,i1,i2,norm(1),norm(2)
                  eqn = neq
                  UVCF(0,0,0,norm(1),uc,rc,RHS)
                  UVCF(0,0,0,norm(2),vc,rc,RHS)

                  eqn = teq
                  UVCF(0,0,0,tang(1),uc,rc,RHS)
                  UVCF(0,0,0,tang(2),vc,rc,RHS)

                  rhs(i1,i2,i3,neq) = rhs(i1,i2,i3,neq) + scale*uvel*rho
...
c     boundary point 
                  rr = 2d0      !2d0*side-1d0
                  if ( isaxi.gt.0 ) rr = rr*x(i1l,i2l,i3l,radaxis)               
                  UVCF(is1,is2,is3,rr*aj(i1l,i2l,i3i)*rsxy(i1l,i2l,i3l,axis,0),rc,uc,RHS)
                  UVCF(is1,is2,is3,rr*aj(i1l,i2l,i3i)*rsxy(i1l,i2l,i3l,axis,1),rc,vc,RHS)
                  fac = 1d0
...
c     ghost ghost point
                     ...
                     UVCF(-is1,-is2,-is3,rr*aj(i1g-is1,i2g-is2,i3-is3)*rsxy(i1g-is1,i2g-is2,i3-is3,axis,0),rc,uc,RHS)
                     UVCF(-is1,-is2,-is3,rr*aj(i1g-is1,i2g-is2,i3-is3)*rsxy(i1g-is1,i2g-is2,i3-is3,axis,1),rc,vc,RHS)
c     ghost point
                     rr = -2d0*alpha*(1d0-2d0*side) !(2d0*side-1d0)*alpha
                     if ( isaxi.gt.0 ) then 
                        rr = rr*x(i1g,i2g,i3g,radaxis)               
...
c                     rhs(i1,i2,i3,neq) = 0
                     if ( isaxi.gt.0 .and.ghostrpos ) 
     &                    rr = rr*x(i1g,i2g,i3g,radaxis)
                     UVCF(0,0,0,rr*aj(i1g,i2g,i3g)*rsxy(i1g,i2g,i3g,a,0),rc,uc,RHS)
                     UVCF(0,0,0,rr*aj(i1g,i2g,i3g)*rsxy(i1g,i2g,i3g,a,1),rc,vc,RHS)
                     if ( bc(side,axis).eq.slipWall 
     &                    .or. bc(side,axis).eq.noSlipWall ) then
                        rr = 1d0
                     else
                     ...
                     if ( isaxi.gt.0 .and. ghostrpos ) 
     &                    rr = rr*x(i1i,i2i,i3i,radaxis)               
                     UVCF( 2*is1, 2*is2, 2*is3,rr*aj(i1i,i2i,i3i)*rsxy(i1i,i2i,i3i,a,0),rc,uc,RHS)
                     UVCF( 2*is1, 2*is2, 2*is3,rr*aj(i1i,i2i,i3i)*rsxy(i1i,i2i,i3i,a,1),rc,vc,RHS)
                  endif
...
c     enddo ! each vel component
c     apply the trick that reduces the chance of a zero pivot (to match coeff array)
                     PIVOTTRICK(neq,teq,RHS) ! pivot trick for boundary equations
                  endif
...
c               endif
               if ( .false. .and. bc(side,axis).eq.subsonicoutflow) then
