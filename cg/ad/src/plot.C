...
setPlotTitle(const real &t, const real &dt)
{
  ...
  else
  {
    ...
    aString advectionLabel;

    if( !variableAdvection)
    {
      if( numberOfDimensions==2 )
        sPrintF(advectionLabel,"a=%4.1g, b=%4.1g, ",a[0],b[0]);
      else
        sPrintF(advectionLabel,"a=%4.1g, b=%4.1g, c=%4.1g, ",a[0],b[0],c[0]);
    }
    else
    {
  ...
  if( option & 1 )
  {
    ...
    if( ! (option & 2) || programHalted )
    {
      ...
      for(;;)
      {
        ...
        else if( answer=="advection streamlines" )
        {
          // --- plot streamlines of the advection velocity ---

          const bool & variableAdvection = parameters.dbase.get<bool >("variableAdvection");
          if( variableAdvection )
          {
            realCompositeGridFunction*& pAdvectVar= parameters.dbase.get<realCompositeGridFunction*>("advectVar");
            assert( pAdvectVar!=NULL );
            // (*pAdvectVar).display("advection","%4.1f ");

            const int uc=0, vc=1;
            psp.set(GI_U_COMPONENT_FOR_STREAM_LINES,uc);
            psp.set(GI_V_COMPONENT_FOR_STREAM_LINES,vc);
            PlotIt::streamLines(ps,*pAdvectVar,psp);
            ps.erase();

          }
          else
          {
             printF("cgad::INFO: The advection is not variable. No streamlines plotted\n");
          }
          plotObjects=true; // replot objects
        }
        else if( answer=="advection contours" )
        {
          // --- plot contours of the advection velocity ---
          ...
          if( variableAdvection )
          {
            ...
            PlotIt::contour(ps,*pAdvectVar,psp);
            ps.erase();

          }
          else
          {
             printF("cgad::INFO: The advection is not variable. No contours are plotted\n");
          }
          plotObjects=true; // replot objects
        }
