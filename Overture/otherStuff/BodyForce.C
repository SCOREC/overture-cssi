                		    ...
                		    GraphicsParameters & pspGlobal )
{
  ...
  // Here is a GraphicsParameters for saving properties of the body and boundary forcing
    ...
    // set initial values
        ...
        {
      // 3D -- what should we plot ?
            pspBoundary.getBoundaryColourOption()=GraphicsParameters::colourByGrid;
            pspBoundary.set(GI_GRID_LINE_COLOUR_OPTION,GraphicsParameters::colourByGrid);
            pspBoundary.set(GI_PLOT_SHADED_MAPPING_BOUNDARIES,false);   // 2d grid lines: OFF
            pspBoundary.set(GI_PLOT_LINES_ON_MAPPING_BOUNDARIES,true); // 3d lines : OFF
        }
  ...
  // --- Build the sibling dialog for colour dialog ---
	...
	// Here is the array of body forcings:
        ...
        // printF("***plotBodyForcing*** plotRegions: regionType=[%s]\n",(const char*)regionType);
          //  save ID for picking:
                    if( !bodyForce.dbase.has_key("globalID") ) bodyForce.dbase.put<int>("globalID",-1); 
            ...
            //   [(x-xe)/ae]^2 + [(y-ye)/be]^2 + [(z-ze)/ce]^2 = 1
              ...
              //printF("BodyForce: plot ellipsoid surface: xe=%e, ye=%e, ze=%e, ae=%e, be=%e, ce=%e lines=%d,%d\n",
              //	     xe,ye,ze,ae,be,ce,linesToPlot[0],linesToPlot[1]);
                            if( ae==be && be==ce )
                            {
        	// Sphere:
                ...
                //                );
                                real outerRadius=ae+1;
                                SphereMapping map( ae,outerRadius,xe,ye,ze );
                      	globalID=map.getGlobalID();  // save ID for picking 
                      	map.setIsSurface( true );
                      	map.setGridDimensions(axis1,linesToPlot[0]);
                      	map.setGridDimensions(axis2,linesToPlot[1]);
                      	PlotIt::plot(gi, map,psp);      
                            }
                            ...
                            {
        	// Ellipse 
                      	CrossSectionMapping map(0.,1., 0.,1., 1., 1.5, xe,ye,ze,1.,3 );
                      	globalID=map.getGlobalID();  // save ID for picking 
                      	map.setCrossSectionType( CrossSectionMapping::ellipse );
                      	map.setEllipsoidParameters( ae,be,ce );
                      	map.setIsSurface( true );
                      	map.setGridDimensions(axis1,linesToPlot[0]);
                      	map.setGridDimensions(axis2,linesToPlot[1]);
        	// map.setGridDimensions(axis3,linesToPlot[2]);
                      	if( false )
                      	{
