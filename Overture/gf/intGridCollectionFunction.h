/*  -*-Mode: c++; -*-  */
#ifndef INT_GRID_COLLECTION_FUNCTION
#define INT_GRID_COLLECTION_FUNCTION "intGridCollectionFunction.h"
...
class intGridCollectionFunction : public intGenericGridCollectionFunction
{
 public:

  enum
  {
    undefinedValue=intMappedGridFunction::undefinedValue,
    defaultValue=intMappedGridFunction::defaultValue,
    forAll=intMappedGridFunction::forAll,
    forAllGrids=forAll+1,
    maximumNumberOfIndicies=intMappedGridFunction::maximumNumberOfIndicies,
    maximumNumberOfComponents=intMappedGridFunction::maximumNumberOfComponents
  };
  
  enum updateReturnValue  // the return value from updateToMatchGrid is a mask of the following values
  {
    updateNoChange          = intMappedGridFunction::updateNoChange, // no changes made
    updateReshaped          = intMappedGridFunction::updateReshaped, // grid function was reshaped
    updateResized           = intMappedGridFunction::updateResized,  // grid function was resized
    updateComponentsChanged = intMappedGridFunction::updateComponentsChanged  // component dimensions may 
...
//  IntegerArray positionOfCoordinate;
  ...
  intGridCollectionFunction operator()(const Range & component0,
				    const Range & component1=nullRange,
				    const Range & component2=nullRange,
				    const Range & component3=nullRange,
				    const Range & component4=nullRange );
  ...
  virtual int dataCopy( const intGridCollectionFunction & gcf );  // copy the array data only
  ...
  virtual updateReturnValue updateToMatchGridFunction(const intGridCollectionFunction & cgf, 
				const Range & R0, 
				const Range & R1=nullRange,
				const Range & R2=nullRange,
				const Range & R3=nullRange,
...
//   int zeroUnusedPoints(intGridCollectionFunction & coeff, 
//                        int value = 0,
// 		       const Index & component0=nullIndex,
// 		       const Index & component1=nullIndex,
// 		       const Index & component2=nullIndex,
...
// 		       const Index & component4=nullIndex );
  ...
  friend intGridCollectionFunction & abs( const intGridCollectionFunction & cgf );
...
#undef INTEGRAL_TYPE
  ...
  friend intGridCollectionFunction & multiply( const intGridCollectionFunction & a, const intGridCollectionFunction & coeff );
  ...
  friend intGridCollectionFunction evaluate( intGridCollectionFunction & u );

 private:
  
  // These are used by list's of ReferenceCounting objects
  virtual void reference( const ReferenceCounting & mgf )
  { intGridCollectionFunction::reference( (intGridCollectionFunction&) mgf ); }
  virtual ReferenceCounting & operator=( const ReferenceCounting & mgf )
  { return intGridCollectionFunction::operator=( (intGridCollectionFunction&) mgf ); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
  { return ::new intGridCollectionFunction(*this,ct); }  
...
//     IntegerArray positionOfCoordinate;
     ...
     int faceCentering;
     Range R[intMappedGridFunction::maximumNumberOfIndicies+1];
     ...
     ListOfIntGridCollectionFunction multigridLevel; //  Collections of grid functions having the same multigrid level.
     ListOfIntGridCollectionFunction refinementLevel; //  Collections of grid functions having the same refinement level.
    private:
