...
class ListOfParentChildSiblingInfo;
...
class GridCollectionData:
  ...
  public:
    enum {
      ...
      ENDtheGridCollectionData   = ENDtheGenericGridCollectionData, // bit 25
    ...
    RCVector<GridCollection> domain;          // (overloaded)
#else
    ...
    ListOfGridCollection     domain;          // (overloaded)
#endif // USE_STL
    Interpolant*             interpolant;

    ListOfParentChildSiblingInfo *parentChildSiblingInfoList;
...
// ********
    ...
    virtual Integer put(GenericDataBase& db,
                        const aString&   name,
                        int geometryToPut = -1    // by default put computedGeometry
                        ) const;
...
//
    int updateRefinementGrid( int n, int b, int p,   
			      const IntegerArray& range,  
			      const IntegerArray& factor, 
			      const Integer& level );
...
//  Collections of grids in the same domain
    RCVector<GridCollection> domain;          // (overloaded)
#else
//  Lists of component grids.
    ListOfMappedGrid         grid;            // (overloaded)
...
//  display parallel distribution 
    void displayDistribution(const aString & label, FILE *file=stdout );

    int numberOfGridPoints() const;  // return the number of grid points
...
//
    ...
    // get the list of parent/child/siblings for AMR
    ListOfParentChildSiblingInfo* getParentChildSiblingInfo() const;

    // update the parent/child/sibling info for AMR
    virtual void updateParentChildSiblingInfo();

    // Call this function when the grid collection has changed
...
//  Specify the grids that belong to a domain
